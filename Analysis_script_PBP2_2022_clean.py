...
for exp in exps[:]:
    ...
    plt.savefig(r'C:\Users\Franc\Data\RodComplex\Results\PBP2_2022_3states/Fig_random_labeled_tracks/' + exp.rsplit('\\')[-1] + '.png')
    plt.savefig(r'C:\Users\Franc\Data\RodComplex\Results\PBP2_2022_3states/Fig_random_labeled_tracks/' + exp.rsplit('\\')[-1] + '.svg')
    ...
    for i in range(nb_rows):
        for j in range(nb_rows):
            if i*nb_rows+j<len(tracks):
                ...
                plt.scatter(track[0,0], track[0,1] , c = 'k', s = 8, marker = 'x')
    plt.gca().set_aspect('equal', adjustable='box')
    plt.savefig(r'C:\Users\Franc\Data\RodComplex\Results\PBP2_2022_3states/Fig_random_labeled_long_tracks/' + exp.rsplit('\\')[-1] + '.png')
    plt.savefig(r'C:\Users\Franc\Data\RodComplex\Results\PBP2_2022_3states/Fig_random_labeled_long_tracks/' + exp.rsplit('\\')[-1] + '.svg')
...
for exp in exps[:]:
    ...
    model.save_weights(r'C:\Users\Franc\Data\RodComplex\Results\PBP2_2022_3states_fixed_params/' + exp.rsplit('\\')[-1]  + '_weights.tf')
...
'''
all_params = []
exps = glob(r'C:\Users\Franc\Data\RodComplex\PBP2_2022\All_60ms\C*')[:-6]
exp = exps[1]
for exp in exps[:]:
    model.load_weights(r'C:\Users\Franc\Data\RodComplex\Results\PBP2_2022_3states_fixed_params/' + exp.rsplit('\\')[-1]  + '_weights.tf')
    params = exatrack.get_model_params(model, track_segmentation = True)
    equ_fractions = exatrack.equilibrium_distribution(params['transition rates'])
    params['equilibrium fractions'] = equ_fractions
    all_params.append(params)
    ...
    for i in range(nb_rows):
        for j in range(nb_rows):
            if i*nb_rows+j<len(tracks):
                ...
                plt.scatter(track[0,0], track[0,1] , c = 'k', s = 8, marker = 'x')
    plt.gca().set_aspect('equal', adjustable='box')
    plt.savefig(r'C:\Users\Franc\Data\RodComplex\Results\PBP2_2022_3states_fixed_params/Fig_random_labeled_long_tracks/' + exp.rsplit('\\')[-1] + '.png')
    plt.savefig(r'C:\Users\Franc\Data\RodComplex\Results\PBP2_2022_3states_fixed_params/Fig_random_labeled_long_tracks/' + exp.rsplit('\\')[-1] + '.svg')
    ...
    for i in range(nb_rows):
        for j in range(nb_rows):
            if i*nb_rows+j<len(tracks):
                ...
                plt.scatter(track[0,0], track[0,1] , c = 'k', s = 8, marker = 'x')
    plt.gca().set_aspect('equal', adjustable='box')
    plt.savefig(r'C:\Users\Franc\Data\RodComplex\Results\PBP2_2022_3states_fixed_params/Fig_random_labeled_tracks/' + exp.rsplit('\\')[-1] + '.png')
    plt.savefig(r'C:\Users\Franc\Data\RodComplex\Results\PBP2_2022_3states_fixed_params/Fig_random_labeled_tracks/' + exp.rsplit('\\')[-1] + '.svg')




# Model fitting with fixed state parameters from WT except for the parameters of the directed state
exps = glob(r'C:\Users\Franc\Data\RodComplex\PBP2_2022\All_60ms\C*')
exp = exps[12]
for exp in exps[:]:
    paths = glob(exp + '/*.csv')
    ...
    model.save_weights(r'C:\Users\Franc\Data\RodComplex\Results\PBP2_2022_3states_fixed_params_except0/' + exp.rsplit('\\')[-1]  + '_weights.tf')
...
exp = exps[1]
for exp in exps[:]:
    model.load_weights(r'C:\Users\Franc\Data\RodComplex\Results\PBP2_2022_3states_fixed_params_except0/' + exp.rsplit('\\')[-1]  + '_weights.tf')
    params = exatrack.get_model_params(model, track_segmentation = True)
    equ_fractions = exatrack.equilibrium_distribution(params['transition rates'])
    params['equilibrium fractions'] = equ_fractions
    All_params.append(params)
...
groups = np.array([int(exp.rsplit('\\')[-1][1]) for exp in exps])
groups[9:11] = groups[8]
np.array(exps)[groups ==2]

compare_params(All_params, groups, labels)

"""
Get the refined position and anomalous variable estimates
"""
...
for exp in exps[:]:
    ...
    tracks, frames, track_IDs, opt_metrics = exatrack.read_table(paths, # path of the file to read or list of paths to read multiple files.
                   lengths = np.arange(50, 301), # number of positions per track accepted (take the first position if longer than max
                   dist_th = np.inf, # maximum distance allowed for consecutive positions 
                   frames_boundaries = [-np.inf, np.inf], # min and max frame values allowed for peak detection
                   fmt = 'csv', # format of the document to be red, 'csv' or 'pkl', one can also just specify a separator e.g. ' '. 
                   colnames = ['POSITION_X', 'POSITION_Y', 'POSITION_T', 'TRACK_ID'],  # if multiple columns are required to identify a track, the string used to identify the track ID can be replaced by a list of strings represening the column names e.g. ['TRACK_ID', 'Movie_ID']
    ...
    #vary_transition_rates[:2, :2] = 0
    tf.math.softmax(transition_rates)
    batch_size = np.min([100, len(tracks)])
    nb_batches = len(tracks)//batch_size
    device = '/GPU:0'
...
"""
Plot labeled tracks with real position and velocity vector estimates 
""" 

def plot_track(ID, tracks, preds, pos_mean, anomalous_mean, masks, colors=None, nb_states=3, dt=1):
    if colors is None:
        colors = np.array([[1, 0, 0], [0, 1, 0], [0, 0, 1]])[:nb_states]
...
model, pred_model = exatrack.build_segment_model(segment_length, # maximum number of time points in the input tracks 
                ...
                transition_shapes, # transition shapes for each pair of states (gamma distributed transition lifetimes)
                initial_fractions, 
                batch_size = 400, # number of tracks analysed at the same time
                nb_dims = nb_dims, # Number of dimensions of the tracks
                sequence_length = sequence_length, # sequence of the previous states that are considered without alterations (computation time and memory usage proportional to sequence_length)
                max_linking_distance = max_linking_distance, # Maximum linking distance or standard deviation for the expected misslinking distance.
                estimated_density = estimated_density, # Estimated density of the sample.
...
for exp in exps[:]:
    ...
    pos_mean = pos_mean[masks[:, 0] == 1]
    anomalous_mean = anomalous_mean[masks[:, 0] ==1]
    ...
    for i in range(nb_rows):
        for j in range(nb_rows):
            if i*nb_rows+j<len(tracks):
                ...
                track = tracks[ID]
                print(len(track))
                track = track - np.mean(track,0 , keepdims = True) + [[lim*i, 1.3*lim*j]]
                p = preds[ID, mask.astype(bool)][:,:-1]
                plt.plot(track[:,0], track[:,1], ':k', alpha = 0.5)
                plt.scatter(track[:,0], track[:,1] , c = p@colors, s = 7)
                plt.scatter(track[0,0], track[0,1] , c = 'k', s = 8, marker = 'x')
    ...
    for i in range(nb_rows):
        for j in range(nb_rows):
            if i*nb_rows+j<len(tracks):
                ID = IDs[i*nb_rows+j]
                mask = masks[ID]
                track = pos_mean[ID][mask.astype(bool)[1:]]
                mask.astype(bool).shape
                pos_mean.shape
                #track = anomalous_mean[ID][mask.astype(bool)[1:]]
                print(len(track))
                track = track - np.mean(track,0 , keepdims = True) + [[lim*(i), 1.3*lim*(j-0.3)]]
                p = preds[ID, mask.astype(bool)][1:,:-1]
                plt.plot(track[:,0], track[:,1], ':k', alpha = 0.5)
                plt.scatter(track[:,0], track[:,1] , c = p@colors, s = 7)
                plt.scatter(track[0,0], track[0,1] , c = 'k', s = 8, marker = 'x')
    plt.gca().set_aspect('equal', adjustable='box')
    ...
    for ID in range(len(tracks)):
        ...
        plt.savefig(r'C:\Users\Franc\Data\RodComplex\Results\PBP2_2022_3states_fixed_params\Fig_track_velocities/' + exp.rsplit('\\')[-1] +str(ID)+ '.png')
        plt.savefig(r'C:\Users\Franc\Data\RodComplex\Results\PBP2_2022_3states_fixed_params\Fig_track_velocities/' + exp.rsplit('\\')[-1] +str(ID) + '.svg')
        plt.close()
