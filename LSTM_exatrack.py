...
'''
# ============================================================================
#  LSTM "constraint function" — PER-STATE-SEQUENCE variant
#  ----------------------------------------------------------------------------
#  Each state-sequence (hypothesis) determines its OWN per-step Gaussian
#  parameters from its OWN running latent state.  There are no connections
#  between different state-sequences in the LSTM front-end:
#
#    * the hypothesis axis  M = nb_states * sequence_length  is folded into the
#      LSTM batch, so the cell / head are applied independently per
#      (track, hypothesis);
#    * the carry-over (h, c) state is one latent per (track, hypothesis);
#    * candidate parameters are SOURCE-hypothesis keyed (tf.repeat over the
#      destination axis), so a candidate p = j*S + b uses hypothesis j = p//S;
#    * the carried LSTM state is mixed across parents with the SAME convex
#      weights used to collapse the analytic coefficients (IMM-style mixing),
#      which is the only coherent way to thread a recurrent latent through the
#      marginalisation.
#
#  The analytic integrator (RNN_reccurence_formula, get_sequences schedules,
#  transition_RNN_reccurence_formula) is reused unchanged.
# ============================================================================
class LSTM_constraint_function(tf.keras.layers.Layer):
    """
    Deep LSTM that produces, per recurrence step, the same structured Gaussian
    parameters as `constraint_function_arbitrary_KF`, but conditioned on the
    running latent state of each *state-sequence* (hypothesis) independently.
    ...
    Hypothesis axis
    ---------------
        M = nb_states * sequence_length   (one latent / one param-set each)

    Parameter packing produced by the head (per track, per hypothesis)
    ------------------------------------------------------------------
    recurrent block  (length L_rec):
        [ Acur        : H*H          dense transition current-var coefs
          dyn_next_off: H*(H-1)/2    strictly-lower next-var coefs (diag = 1)
    ...
    def __init__(self,
        ...
        # number of state-sequences (hypotheses); folded into the LSTM batch
        self.M = nb_states * sequence_length
    ...
    # ------------------------------------------------------------------ #
    def _feature_dim(self):
        """Per-hypothesis feature size (NOT flattened over hypotheses)."""
        H = self.nb_hidden_vars
        S = self.nb_states
        D = self.nb_dims
        pc = H * H            # Prev_coefs  for this hypothesis        (H, H)
        pb = H * D            # Prev_biases for this hypothesis        (H, D)
        lp = 1               # LP          for this hypothesis
        sl = 1               # segment_len for this hypothesis
        gm = S               # gamma_dist_mean (S destinations)
        gv = S               # gamma_dist_var  (S destinations)
        return pc + pb + lp + sl + gm + gv
    ...
    def build(self, input_shape):
        ...
        self.cells = [tf.keras.layers.LSTMCell(self.lstm_units, dtype=d,
                                               name='lstm_cell_%d' % i)
                      for i in range(self.nb_lstm_layers)]
        # ONE parameter set per LSTM-batch element (= per hypothesis)
        self.head = tf.keras.layers.Dense(
            self.L_rec + self.L_init, dtype=d, name='param_head')
        ...
        # learned "start token" used as LSTM input at t = 0 (no prev state yet);
        # one token PER hypothesis, state-fast ordering (matches states_indices)
        self.init_token = self.add_weight(
            name='init_token', shape=(self.M, self.feature_hidden),
            initializer='zeros', dtype=d, trainable=True)

        # cross-batch carry-over of the LSTM (h, c) for every layer, with the
        # hypothesis axis folded into the batch: (batch_size * M, lstm_units)
        if self.carryover:
            BM = self.batch_size * self.M
            self.carry_h = [
                tf.Variable(np.zeros((BM, self.lstm_units)),
                            dtype=d, trainable=False, name='carry_h_%d' % i)
                for i in range(self.nb_lstm_layers)]
            self.carry_c = [
                tf.Variable(np.zeros((BM, self.lstm_units)),
    ...
    def zero_state(self):
        d = self.dtype
        BM = self.batch_size * self.M
        return [[tf.zeros((BM, self.lstm_units), dtype=d),
                 tf.zeros((BM, self.lstm_units), dtype=d)]
                for _ in range(self.nb_lstm_layers)]
    ...
    def featurize(self, Prev_coefs, Prev_biases, LP,
                  segment_len, gamma_dist_mean, gamma_dist_var):
        """
        Build the LSTM input PER HYPOTHESIS, then fold the hypothesis axis into
        the batch -> (N*M, feat_dim).  No information from one hypothesis ever
        enters another's feature vector.
        ...
        Shapes in:
            ...
            LP              (N, M)
            segment_len     (N, M)
            gamma_dist_mean (N, M*S)   hypothesis-major, destination-fast
            gamma_dist_var  (N, M*S)
        """
        N = self.batch_size
        M = self.M
        S = self.nb_states
        sg = tf.stop_gradient if self.stop_gradient_features else tf.identity

        pc = tf.reshape(tf.transpose(sg(Prev_coefs),  [1, 2, 0, 3]), (N, M, -1))  # (N,M,H*H)
        pb = tf.reshape(tf.transpose(sg(Prev_biases), [1, 2, 0, 3]), (N, M, -1))  # (N,M,H*D)

        # NOTE: the max over hypotheses is only numerical centering.  Detach it
        # so the (shared) constant cannot couple hypotheses through the
        # gradient.  Drop the `lp` feature entirely for STRICTLY zero coupling.
        lp = (LP - tf.stop_gradient(tf.reduce_max(LP, axis=1, keepdims=True)))[..., None]  # (N,M,1)
        sl = segment_len[..., None]                                                        # (N,M,1)
        gm = tf.reshape(gamma_dist_mean, (N, M, S))                                        # (N,M,S)
        gv = tf.reshape(gamma_dist_var,  (N, M, S))                                        # (N,M,S)

        feat = tf.concat([pc, pb, lp, sl, gm, gv], axis=2)        # (N, M, feat_dim)
        return tf.reshape(feat, (N * M, -1))                      # (N*M, feat_dim)
    ...
    def step(self, features, states):
        ...
        out, new_states = self._run_cells(x, states)
        raw = self.head(out)
        raw = tf.reshape(raw, (self.batch_size, self.M,
                               self.L_rec + self.L_init))
        return raw, new_states

    def step_initial(self, states):
        N, M = self.batch_size, self.M
        x = tf.reshape(
            tf.broadcast_to(self.init_token[None], (N, M, self.feature_hidden)),
            (N * M, self.feature_hidden))
        out, new_states = self._run_cells(x, states)
        raw = self.head(out)
        raw = tf.reshape(raw, (N, M, self.L_rec + self.L_init))
        return raw, new_states

    # ------------------------------------------------------------------ #
    def _unit_lower(self, flat, n, idx_map, mask):
        """(N, M, n*(n-1)/2) strictly-lower params -> (N, M, n, n) unit-lower."""
        d = self.dtype
        N = self.batch_size
        M = self.M
        if n * (n - 1) // 2 > 0:
            ...
            off = tf.zeros((N, M, n, n), dtype=d)
        eye = tf.eye(n, batch_shape=[N, M], dtype=d)
        return off + eye

    # ------------------------------------------------------------------ #
    def assemble(self, raw, nb_dims):
        """
        Turn the raw (N, M, L_rec+L_init) LSTM output into the structured,
        std-normalised, *one time step* Gaussian tensors expected downstream.
        The "state" axis is now the hypothesis axis M.

        Returns a dict with recurrent / initial / transition coefficients,
        biases and (nb_dims-scaled) log-normalisers.  Shapes:
            rec_hidden_cur   (n_g, N, M, H)
            rec_hidden_next  (n_g, N, M, H)
            rec_obs          (n_g, N, M, O)
            rec_biases       (n_g, N, M, nb_dims)
            Log_factors      (N, M)
            init_hidden      (H,  N, M, H)
            init_obs         (H,  N, M, O)
            init_biases      (H,  N, M, nb_dims)
            initial_Log_factors (N, M)
            trans_hidden     (n_trans, N, M, H)
            trans_biases     (n_trans, N, M, nb_dims)
            transition_Log_factors (N, M)
        """
        d = self.dtype
        N = self.batch_size
        S = self.M                       # <-- hypothesis axis replaces nb_states
        H = self.nb_hidden_vars
        O = self.nb_obs_vars
        idx = self.integration_variable_index
        n_sl_H = self.n_sl_H
        ...
        Acur = tf.reshape(A_flat, (N, S, H, H))
        meas_hidden = tf.reshape(meas_hidden, (N, S, O, H))
        LOG_STD_MIN = tf.math.log(tf.constant(1e-10, dtype=d))
        LOG_STD_MAX = tf.math.log(tf.constant(1e10,  dtype=d))
        log_stds = LOG_STD_MIN + (LOG_STD_MAX - LOG_STD_MIN) * tf.math.sigmoid(log_stds)
        stds = tf.math.exp(log_stds)
        ...
        # ---- recurrent hidden coefs  [current(H) | next(H)] ----
        dyn_rows = tf.concat([Acur, dyn_next_coefs], axis=-1)      # (N,M,H,2H)
        meas_next = tf.zeros((N, S, O, H), dtype=d)
        meas_rows = tf.concat([meas_hidden, meas_next], axis=-1)   # (N,M,O,2H)
        C = tf.concat([dyn_rows, meas_rows], axis=2)              # (N,M,n_g,2H)
        hidden = tf.transpose(C, [2, 0, 1, 3])                    # (n_g,N,M,2H)

        obs_dyn = tf.zeros((H, N, S, O), dtype=d)
        obs_meas = tf.transpose(meas_obs, [2, 0, 1, 3])          # (O,N,M,O)
        obs = tf.concat([obs_dyn, obs_meas], axis=0)             # (n_g,N,M,O)

        gbias = tf.transpose(biases_p, [2, 0, 1])                # (n_g,N,M)
        rbias = tf.broadcast_to(gbias[..., None], (n_g, N, S, nb_dims))
        gstd = tf.transpose(stds, [2, 0, 1])                     # (n_g,N,M)
        gstd = gstd[..., None]                             # (n_g,N,M,1) ->broadcast
        ...
        Log_factors = -tf.reduce_sum(tf.math.log(stds), axis=-1)  # (N,M)

        # ---- initial / prior gaussians ----
        init_hidden = tf.transpose(L, [2, 0, 1, 3])              # (H,N,M,H)
        init_obs = tf.zeros((H, N, S, O), dtype=d)
        gistd = tf.transpose(init_std, [2, 0, 1])[..., None]   # (H,N,M,1)
        gibias = tf.transpose(init_bias, [2, 0, 1])
        init_biases = tf.broadcast_to(gibias[..., None], (H, N, S, nb_dims))
        init_hidden = init_hidden / gistd
        init_obs = init_obs / gistd
        init_biases = init_biases / gistd
        initial_Log_factors = -tf.reduce_sum(tf.math.log(init_std), axis=-1)  # (N,M)

        # ---- transition gaussians (fresh prior on vars idx..H-1) ----
        TL = L[:, :, idx:H, :]                                   # (N,M,n_trans,H)
        trans_hidden = tf.transpose(TL, [2, 0, 1, 3])            # (n_trans,N,M,H)
        t_std = init_std[:, :, idx:H]
        gtstd = tf.transpose(t_std, [2, 0, 1])[..., None]  # (n_trans,N,M,1)
        t_bias = init_bias[:, :, idx:H]
        gtbias = tf.transpose(t_bias, [2, 0, 1])
        trans_biases = tf.broadcast_to(gtbias[..., None],
                                       (self.n_trans, N, S, nb_dims))
        ...
        trans_biases = trans_biases / gtstd
        transition_Log_factors = -tf.reduce_sum(
            tf.math.log(init_std[:, :, idx:H]), axis=-1)         # (N,M)
...
#  LSTM-driven single recurrence step (front-end swapped, integrator unchanged)
#  ----------------------------------------------------------------------------
#  Per-step Gaussian factors are produced PER HYPOTHESIS and expanded to the
#  P = seq*S*S candidate axis with tf.repeat over the destination axis, so each
#  candidate p uses ONLY the source hypothesis j = p // nb_states.  The carried
#  LSTM state is mixed across parents with the same convex weights used to
#  collapse the analytic coefficients.
# ============================================================================
@tf.function(jit_compile=False)
def RNN_cell_LSTM(input_i, Prev_coefs, Prev_biases, LP, segment_len,
                  lstm_module, lstm_states,
    ...
    """
    Identical bookkeeping to the original `RNN_cell`, except the per-step
    Gaussian factors are produced by `lstm_module` PER HYPOTHESIS and the
    carried (h, c) state is remixed across parents.  Returns the updated
    carriers plus the new LSTM (h, c) state list.

    `oh_row` / `oh_col` are no longer used for keying the LSTM parameters
    (candidates are SOURCE-hypothesis keyed via tf.repeat); they are kept in the
    signature only so the loop body call site stays unchanged.  The Gamma
    dwell-time machinery that uses them lives in `Custom_RNN_LSTM_layer`.
    """
    nb_dims = input_i.shape[-1]
    nb_tracks = LP.shape[0]
    nb_hidden_vars = Prev_coefs.shape[3]
    nb_states = lstm_module.nb_states
    sequence_length = LP.shape[1] // nb_states     # = seq ; M = LP.shape[1]
    ...
    reccurent_obs_var_coefs         = g['rec_obs']           # (n_g,N,M,O)
    reccurent_hidden_var_coefs      = g['rec_hidden_cur']    # (n_g,N,M,H)
    reccurent_next_hidden_var_coefs = g['rec_hidden_next']   # (n_g,N,M,H)
    reccurent_biases                = g['rec_biases']        # (n_g,N,M,nb_dims)
    transition_hidden_var_coefs     = g['trans_hidden']      # (n_trans,N,M,H)
    transition_biases               = g['trans_biases']      # (n_trans,N,M,nb_dims)
    Log_factors_NS                  = g['Log_factors']       # (N,M)
    transition_Log_factors_NS       = g['transition_Log_factors']  # (N,M)

    # expand transition coefs to the candidate axis by SOURCE hypothesis
    # (each of the M hypotheses -> its nb_states destination candidates)
    transition_hidden_var_coefs = tf.repeat(
        transition_hidden_var_coefs, nb_states, axis=2)
    transition_biases = tf.repeat(transition_biases, nb_states, axis=2)

    # per-candidate log-normaliser, source-hypothesis keyed (no oh_row/oh_col)
    flat_Log = tf.repeat(Log_factors_NS, nb_states, axis=1)            # (N,P)
    flat_trans = tf.repeat(transition_Log_factors_NS, nb_states, axis=1)
    reshaped_Log_factors = flat_trans * transition_mask + flat_Log * (1 - transition_mask)
    ...
    # expand recurrent coefs to candidates by SOURCE hypothesis (tf.repeat)
    current_reccurent_obs_var_coefs = tf.repeat(
        reccurent_obs_var_coefs, nb_states, axis=2)
    current_reccurent_hidden_var_coefs = tf.repeat(
        reccurent_hidden_var_coefs, nb_states, axis=2)
    current_reccurent_next_hidden_var_coefs = tf.repeat(
        reccurent_next_hidden_var_coefs, nb_states, axis=2)
    current_reccurent_biases = tf.repeat(
        reccurent_biases, nb_states, axis=2)
    ...
    # ------------------------------------------------------------------ #
    #  IMM-style mixing of the carried LSTM state.  The state arrives indexed
    #  by the M *incoming* hypotheses, folded as (N*M, U).  We apply EXACTLY the
    #  same transition / stable / fuse-last collapse used for `Next_coefs`, so
    #  each outgoing hypothesis inherits the convex mixture of its parents'
    #  latents.  No latent from one outgoing sequence leaks into another beyond
    #  this principled mixing.
    # ------------------------------------------------------------------ #
    def _reduce_lstm(state):
        U = state.shape[-1]
        s = tf.reshape(state, (nb_tracks, sequence_length * nb_states, U))   # (N, M, U)
        s = tf.repeat(s, nb_states, axis=1)                                  # (N, P, U) source-keyed
        grid = tf.reshape(s, (nb_tracks, sequence_length * nb_states,
                              nb_states, U))                                 # (N, M, S, U)
        st = tf.reduce_sum(grid * transition_weights[:, :, :, None], axis=1)  # (N, S, U)
        ss = tf.reduce_sum(grid * stable_weights[:, :, :, None],     axis=2)  # (N, M, U)
        cat = tf.concat([st, ss], axis=1)                                     # (N, S+M, U)
        saved = cat[:, :-nb_states * 2]                                       # (N, M-S, U)
        last = tf.reshape(cat[:, -nb_states * 2:], (nb_tracks, 2, nb_states, U))
        reduced_last = tf.reduce_sum(last * last_weights[:, :, :, None], axis=1)  # (N, S, U)
        out = tf.concat([saved, reduced_last], axis=1)                        # (N, M, U)
        return tf.reshape(out, (nb_tracks * sequence_length * nb_states, U))

    new_lstm_states = [[_reduce_lstm(h), _reduce_lstm(c)] for h, c in new_lstm_states]
...
# ============================================================================
#  Initial layer (t = 0)  — LSTM driven, per hypothesis
# ============================================================================
class Initial_LSTM_layer(tf.keras.layers.Layer):
    """
    First time step.  Reads the LSTM carry-over state (masked by is_first),
    ...
    The LSTM state is folded over the hypothesis axis (N*M, U); there is no
    tiling of the assembled parameters (assemble already emits one set per
    hypothesis).
    """
    ...
    def call(self, inputs, input_dts, input_isfirst):
        ...
        # ---- LSTM initial state: resume from carry if not is_first ----------
        # states are folded as (N*M, U); is_first is broadcast over M (N-major)
        if lstm.carryover:
            base = lstm.carry_state()
        else:
            base = lstm.zero_state()
        zero = lstm.zero_state()
        isf = tf.repeat(input_isfirst, M)[:, None]            # (N*M, 1)
        lstm_states0 = []
        for i in range(lstm.nb_lstm_layers):
            h = isf * zero[i][0] + (1 - isf) * base[i][0]
            c = isf * zero[i][1] + (1 - isf) * base[i][1]
        ...
        # recurrent step-0 gaussians   (axis-2 = M)
        rec_obs = g['rec_obs']
        rec_cur = g['rec_hidden_cur']
        rec_next = g['rec_hidden_next']
        rec_bias = g['rec_biases']
        ...
        init_obs = g['init_obs']
        init_bias = g['init_biases']
        initial_Log_factors = g['initial_Log_factors']        # (N,M)

        # fold first observation into both blocks
        biases = rec_bias + tf.reduce_sum(rec_obs[..., None] * inputs[0], -2)
        init_bias = init_bias + tf.reduce_sum(init_obs[..., None] * inputs[0], -2)

        current_initial = init_hidden                          # (H,N,M,H)
        next_initial = tf.zeros((H, nb_tracks, M, H), dtype=d)

        current_hidden = tf.concat((current_initial, rec_cur), axis=0)
        next_hidden = tf.concat((next_initial, rec_next), axis=0)
        biases = tf.concat((init_bias, biases), axis=0)

        # NO tiling over seq: assemble already emits one param-set per hypothesis
        Next_coefs, Next_biases, LC = RNN_reccurence_formula(
            current_hidden, next_hidden, biases,
            self.initial_sequence_phase_1, self.initial_sequence_phase_2,
            nb_dims, dtype=d)

        # state prior is tiled (state-fast, matches hypothesis ordering);
        # the LSTM log-factors are already per hypothesis (N, M)
        init_log_fractions = tf.concat([tf.math.log(initial_fractions)] * seq, axis=1)
        init_log_factors = initial_Log_factors                 # (N, M)
        LP = LC + init_log_factors + init_log_fractions + tf.math.log(np.array(1 / seq))
...
# ============================================================================
#  Recurrent layer  — LSTM driven, per hypothesis
# ============================================================================
class Custom_RNN_LSTM_layer(tf.keras.layers.Layer):
    """
    Drives `RNN_cell_LSTM` across the track in a tf.while_loop.  The Gamma
    dwell-time machinery (transition_param_function, hazard weighting) is the
    same as in the original layer; only the source of the Gaussian factors
    changed.  The LSTM (h, c) state is a loop variable folded over the
    hypothesis axis (N*M, U) and is carried across batches via the LSTM
    module's carry-over variables.
    """
    ...
    def build(self, input_shape):
        ...
        self.indices = indices
        self.transition_mask = transition_mask
        # kept only for the Gamma dwell-time machinery (NOT for keying LSTM params)
        self.oh_row = tf.cast(tf.one_hot(indices[:, 0], nb_states), dtype)   # (P,S) current
        self.oh_col = tf.cast(tf.one_hot(indices[:, 1], nb_states), dtype)   # (P,S) next
    ...
    def call(self, inputs, input_dts, reference_dt, mask,
        ...
        def body(i, Prev_coefs, Prev_biases, LP, segment_len,
            ...
            mask_scalar = mask_i[:, None]
            mask_state = mask_i[:, None, None, None]
            mask_lstm = tf.repeat(mask_i, lstm.M)[:, None]      # (N*M, 1), N-major
...
def build_segment_model_LSTM(track_len,
    ...
    """
    Same wiring as `build_segment_model`, but the constraint function is the deep
    LSTM operating PER STATE-SEQUENCE.  `params` / `initial_params` are no longer
    needed (the LSTM learns the dynamics); a *template* constraint function is
    used solely to discover the integration schedules via `get_sequences`.
    """
    H = nb_hidden_vars
    O = nb_obs_vars
    nb_gaussians = O + H
...
# -*- coding: utf-8 -*-
"""
Generalised hidden-variable inference for ExaTrack-style segmented state-space
models built with ``constraint_function_arbitrary_KF``.

`extract_hidden_variables` (in the main module) is hard-wired for exactly two
hidden variables (position + one anomalous variable): it integrates the joint
posterior with the explicit two-Gaussian product trick (`marginalise_variable`).
This module replaces that with the equivalent linear-algebra statement, which
works for ANY number H of hidden variables produced by
``constraint_function_arbitrary_KF`` and any number of spatial dimensions.

Maths
-----
For every (track, time step, sequence) the recurrent layer stores the filtered
posterior over the H hidden variables as a product of H *unit-variance* Gaussian
factors

    p(x) ∝ ∏_g  exp( -1/2 * ( Σ_h C[g, h] x_h + b[g, d] )^2 )      (per dim d)

i.e. the multivariate Gaussian  N(mu, Sigma)  with

    mu[:, d] = -C^{-1} b[:, d]                  (one mean vector per dimension)
    Sigma    =  C^{-1} C^{-T} = (C^T C)^{-1}    (shared across dimensions)

so the marginal of hidden variable h is N(mu[h, d], Sigma[h, h]) with

    Sigma[h, h] = Σ_g (C^{-1})[h, g]^2 .

For the 2-variable case this is algebraically identical to what
`marginalise_variable` + `extract_hidden_variables` compute (verified
numerically in the accompanying test), but it never assumes H = 2 and never
needs the triangular structure explicitly (it only needs C to be invertible,
which the arbitrary-KF parameterisation guarantees generically).

Per-sequence weighting
----------------------
The sequences are recombined exactly as in the original: a softmax over the
log-likelihood of each sequence, then mixture moments (E[Var] + Var[E]).
The original uses a DIFFERENT weighting log-likelihood for each hidden variable
(`All_LPs_pos` for position, `All_LPs_ano` for the anomalous variable). Both are
the same general quantity: the marginal likelihood of the sequence with every
hidden variable integrated out *except* the one of interest, whose marginal
Gaussian is left un-normalised. That works out to

    LP_h = All_LPs - nb_dims * ( log|det C| + log sigma_h )

(constants independent of the sequence dropped, since they cancel in the
softmax). This reduces to the original `All_LPs_pos` / `All_LPs_ano`
expressions for h = 0 / h = 1 respectively.

Two recombinations are returned for every variable:
  * ``per_state_*``  : mixes only the ``sequence_length`` parallel histories
                       that share the same current state -> one estimate per
                       state (this is what the original does for the *anomalous*
                       variable, which is state-dependent: velocity / well-anchor).
  * ``collapsed_*``  : mixes every sequence -> a single, state-marginalised
                       estimate (this is what the original does for *position*,
