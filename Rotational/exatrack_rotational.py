# -*- coding: utf-8 -*-
"""
Rotational-diffusion extension for ExaTrack
===========================================

This module adapts the anisotropic-localization ExaTrack script so that the
model jointly describes, per particle and per state:

  * 3-D translational anomalous diffusion (directed or confined), and
  * rotational anomalous diffusion of the particle orientation, described by
    two angles (theta, phi), also directed (persistent rotation) or confined
    (preferred orientation).

The two motions are INDEPENDENT but both dictated by the current hidden state.

------------------------------------------------------------------------------
KEY IDEA (why so little of the core machinery changes)
------------------------------------------------------------------------------
The anisotropic ExaTrack model already treats every spatial dimension as an
INDEPENDENT 1-D channel carrying two hidden variables (position + anomalous
variable). Rotational diffusion of an angle has exactly the same 1-D structure:
the angle plays the role of "position", the angular velocity / preferred
orientation plays the role of the "anomalous variable", and the orientation
measurement error plays the role of the "localization error".

So we simply run the model with

        nb_dims = 5   ->   [ x, y, z, angle1, angle2 ]

where channels 0..2 use the TRANSLATIONAL parameters and channels 3..4 use the
ROTATIONAL parameters. Every Gaussian-product / integration routine
(RNN_gaussian_product, RNN_reccurence_formula, RNN_cell, ...) already broadcasts
over the dimension axis and sums log-factors over it, so nothing there needs to
change. Only the parameter -> per-channel mapping (constraint_function), the
mislinking-state row and a few column indices differ.

------------------------------------------------------------------------------
PER-STATE PARAMETER LAYOUT  (params: shape (nb_states, 13))
------------------------------------------------------------------------------
  col  name         meaning
...
initial_params: shape (nb_states, >=2)
   0   log translational initial spread
   1   log rotational   initial spread   (falls back to col 0 if only 1 column)

------------------------------------------------------------------------------
INTEGRATION NOTE
------------------------------------------------------------------------------
This file is meant to be dropped INTO / imported into the existing anisotropic
ExaTrack script (same namespace). It reuses, unchanged, from that script:
    RNN_reccurence_formula, Initial_layer_constraints, Custom_RNN_layer,
    Final_layer, transpose_layer, IsfirstMaskLayer, CarryoverAssignLayer,
    transition_param_function, dtype, minval, min_lifetime,
    anomalous_diff_3D, simulate_3D_rotational_diffusion, _sample_transitions.

Replace the originals with the same-named objects below; add the new ones
(anomalous_orientation_2D, Initial_layer_constraints_rotational).
"""
...
# Fixed channel layout: 3 translational (x,y,z) + 2 rotational (angle1,angle2).
NB_TRANS_DIMS = 3
NB_ROT_DIMS   = 2
NB_DIMS_TOTAL = NB_TRANS_DIMS + NB_ROT_DIMS   # = 5
...
@njit
def anomalous_orientation_2D(track_len=20,
                             OrientErr=0.0,
                             rot_D=0.05,
                             rot_velocity=None,     # array, length track_len*nb_sub_steps-1
                             rot_conf_force=None,   # array, length track_len*nb_sub_steps-1
                             rot_conf_D=0.0,
                             rot_conf_dist=0.0,
    ...
    """
    Generate the two orientation angles (theta, phi) of a particle under
    state-dependent rotational anomalous diffusion.

    Each angle is an INDEPENDENT 1-D process (matching the per-channel
    independence of the inference model):

      directed rotation : angle[i+1] = angle[i] + rot_velocity[i] + noise
                          (persistent rotation at an angular velocity)
      confined rotation : angle[i+1] = (1-cf)*(angle[i] + noise) + cf*anchor[i]
                          (orientation pulled toward a preferred direction)

    Angles are treated as UNBOUNDED reals (no 2*pi wrapping) so the process is
    consistent with the linear-Gaussian inference model; this is the standard
    small/moderate-angle approximation.

    `noise` has per-sub-step std sqrt(2 * rot_D * dt[i]); the confining anchor
    itself performs a random walk with std sqrt(2 * rot_conf_D * dt[i]) and
    starts a distance ~N(0, rot_conf_dist) from the initial orientation.
    """
    nb_angles = 2
    n_disps = track_len * nb_sub_steps - 1

    angles = np.zeros((track_len * nb_sub_steps, nb_angles))
    angles[0] = initial_angles

    disps = np.zeros((n_disps, nb_angles))
    for i in range(n_disps):
        s = np.sqrt(2.0 * rot_D * dt[i])
        for d in range(nb_angles):
            disps[i, d] = np.random.normal(0.0, s)

    anchors = np.zeros((n_disps, nb_angles))
    for i in range(n_disps):
        s = np.sqrt(2.0 * rot_conf_D * dt[i])
        for d in range(nb_angles):
            anchors[i, d] = np.random.normal(0.0, s)
    for d in range(nb_angles):
        anchors[0, d] = angles[0, d] + np.random.normal(0.0, rot_conf_dist)
    for i in range(1, n_disps):
        for d in range(nb_angles):
            anchors[i, d] += anchors[i - 1, d]

    for i in range(n_disps):
        cf = rot_conf_force[i]
        for d in range(nb_angles):
            angles[i + 1, d] = angles[i, d] + rot_velocity[i] + disps[i, d]
            angles[i + 1, d] = (1.0 - cf) * angles[i + 1, d] + cf * anchors[i, d]

    final = np.zeros((track_len, nb_angles))
    for i in range(track_len):
        final[i] = angles[i * nb_sub_steps]

    if OrientErr > 0:
        final += np.random.normal(0.0, OrientErr, (track_len, nb_angles))
    return final


def anomalous_diff_transition(max_track_len=50,                          # Maximal track length
                              nb_tracks=100,                              # Number of tracks
                              LocErr=0.02,                                # Localization error (position)
                              Fs=np.array([0., 1]),                       # Initial fractions of each state
                              # ---- translational motion ----
                              Ds=np.array([0.0, 0.25]),                   # Diffusion coefficients
                              velocities=np.array([0.03, 0.0]),           # Directed-motion speed
                              angular_Ds=np.array([0.0, 0.0]),            # Angular diffusion of the directed velocity vector
                              conf_forces=np.array([0.0, 0.2]),           # Attraction toward the potential well
                              ...
                              rot_Ds=np.array([0.0, 0.5]),                # Rotational diffusion coefficients
                              rot_velocities=np.array([0.15, 0.0]),       # Persistent angular velocity (directed rotation)
                              rot_conf_forces=np.array([0.0, 0.2]),       # Attraction toward the preferred orientation
                              rot_conf_Ds=np.array([0.0, 0.0]),           # Diffusion of the preferred orientation
                              ...
                              transition_matrix=np.array([[0.00, 0.1],    # Transition rates
                                                          [0.1, 0.00]]),
                              shape_matrix=np.array([[0, 1],              # Transition shapes (gamma dwell times)
                                                     [1, 0]]),
                              bleaching_rate=0.01,                        # Bleaching rate per time step
                              LocErr_std=0.002,                           # Std of the localization error (0 = constant)
                              dt=0.02,                                    # Time step
                              dt_std=0.002,                               # Std of the time steps (0 = constant)
                              ...
                              blur_ratio=0,                               # Motion-blur exposure fraction
                              gap_probability=0):                         # Probability of a missing localization
    """
    Simulate tracks that combine 3-D translational anomalous diffusion with
    rotational anomalous diffusion of the particle orientation.

    Output tracks have 5 columns per time point:  [x, y, z, angle1, angle2].

    The translational and rotational motions are independent processes, each
    directed or confined, both selected by the current state. Rotational
    parameters (rot_*) are indexed by state exactly like the translational ones.

    Returns
    -------
    all_tracks  : (nb_tracks, max_track_len, 5)
    all_LocErrs : (nb_tracks, max_track_len, 5)   (3 position + 2 orientation errors)
    all_dts     : (nb_tracks, max_track_len)
    all_states  : (nb_tracks, max_track_len)
    all_masks   : (nb_tracks, max_track_len)
    """
    blur_cutoff = int(np.ceil(nb_sub_steps * blur_ratio))
    nb_states = len(velocities)

    per_state_arrays = [Fs, Ds, velocities, angular_Ds, conf_forces, conf_Ds,
                        conf_dists, rot_Ds, rot_velocities, rot_conf_forces,
                        rot_conf_Ds, rot_conf_dists, transition_matrix]
    if not np.all(np.array([len(a) for a in per_state_arrays]) == nb_states):
        raise ValueError('All per-state arrays (translational AND rotational) '
                         'must have length nb_states.')
    ...
    all_tracks  = np.zeros((nb_tracks, max_track_len, NB_DIMS_TOTAL))
    all_states  = np.zeros((nb_tracks, max_track_len))
    all_masks   = np.zeros((nb_tracks, max_track_len))
    all_LocErrs = np.zeros((nb_tracks, max_track_len, NB_DIMS_TOTAL))
    ...
    for k in range(nb_tracks):
        ...
        # ---- Cumulative sub-step times (state clock) ----
        ...
        # ---- Initial position, orientation and state ----
        initial_positions = np.random.rand(NB_TRANS_DIMS) * field_of_view
        initial_angles = np.random.rand(NB_ROT_DIMS) * 2 * np.pi
        track, orientations, states = [], [], []
        ...
        while n <= burn_in_subs_total:
            state = next_state
            transitions = _sample_transitions(state, n, cum_sub_times, shape_matrix,
        ...
        if transitions[next_state] == 0:
            state = next_state
            transitions = _sample_transitions(state, burn_in_subs_total, cum_sub_times,
        ...
        while len(track) < track_len_subs:
            if len(track) > 0:
                transitions = _sample_transitions(state, burn_in_subs_total + len(track),
                                                  cum_sub_times, shape_matrix,
                                                  transition_matrix, nb_states, dt)
            l = min(int(np.min(transitions)), track_len_subs - len(track))

            # translational + rotational parameters of the current state
            ...
            rot_D, rot_velocity, rot_conf_force, rot_conf_D, rot_conf_dist = (
                rot_Ds[state], rot_velocities[state], rot_conf_forces[state],
                rot_conf_Ds[state], rot_conf_dists[state])
            ...
            rot_velocity_seg = np.full(l, rot_velocity / nb_sub_steps)
            rot_conf_force_seg = np.full(l, rot_conf_force / nb_sub_steps)

            # position (always 3-D for this model)
            segment = anomalous_diff_3D(track_len=l + 1, LocErr=0, D=D,
                                        velocity=velocity_seg, angular_D=angular_D,
                                        conf_force=conf_force_seg, conf_D=conf_D,
                                        conf_dist=conf_dist, dt=sub_dts_seg,
                                        nb_sub_steps=1, initial_positions=initial_positions)
            # orientation (2 angles)
            orient_segment = anomalous_orientation_2D(track_len=l + 1, OrientErr=0,
                                        rot_D=rot_D, rot_velocity=rot_velocity_seg,
                                        rot_conf_force=rot_conf_force_seg, rot_conf_D=rot_conf_D,
                                        rot_conf_dist=rot_conf_dist, dt=sub_dts_seg,
                                        nb_sub_steps=1, initial_angles=initial_angles)

            track        += list(segment[:-1])
            orientations += list(orient_segment[:-1])
            states       += [state] * l
            initial_positions = segment[-1]
            initial_angles    = orient_segment[-1]

            state = np.argmin(transitions)

        # ---- Measurement errors (3 position + 2 orientation) ----
        if LocErr_std > 0:
            scale = LocErr_std ** 2 / LocErr
            shape = LocErr / scale
            pos_LocErrs = np.random.gamma(shape, scale, (track_len, NB_TRANS_DIMS))
        else:
            pos_LocErrs = np.full((track_len, NB_TRANS_DIMS), LocErr)
        if OrientErr_std > 0:
            oscale = OrientErr_std ** 2 / OrientErr
            oshape = OrientErr / oscale
            ang_LocErrs = np.random.gamma(oshape, oscale, (track_len, NB_ROT_DIMS))
        else:
            ang_LocErrs = np.full((track_len, NB_ROT_DIMS), OrientErr)
        LocErrs = np.concatenate([pos_LocErrs, ang_LocErrs], axis=1)   # (track_len, 5)

        pos_arr = np.array(track)          # (track_len_subs, 3)
        ang_arr = np.array(orientations)   # (track_len_subs, 2)
        combined = np.concatenate([pos_arr, ang_arr], axis=1)          # (track_len_subs, 5)

        if blur_ratio == 0:
            full_track = combined[::nb_sub_steps] + np.random.normal(0, LocErrs, (track_len, NB_DIMS_TOTAL))
            states_arr = np.array(states)[::nb_sub_steps]
        else:
            full_track = np.mean(np.reshape(combined, (track_len, nb_sub_steps, NB_DIMS_TOTAL))[:, :blur_cutoff], 1) \
                         + np.random.normal(0, LocErrs, (track_len, NB_DIMS_TOTAL))
            states_arr = np.mean(np.reshape(np.array(states), (track_len, nb_sub_steps))[:, :blur_cutoff], 1)

        all_tracks[k, :track_len]  = full_track
        all_tracks[k, track_len:]  = full_track[-1]
        all_LocErrs[k, :track_len] = LocErrs
        all_LocErrs[k, track_len:] = LocErrs[-1]
        all_dts[k, :track_len]     = dts
        all_dts[k, track_len:]     = dts[-1]
        all_states[k, :track_len]  = states_arr
        all_states[k, track_len:]  = states_arr[-1]
...
def RNN_gaussian_product(current_hidden_var_coefs_1, current_hidden_var_coefs_2,
    ...
    '''
    # coefs:  (nb_tracks, nb_states, nb_LocErr_dims, nb_hidden_vars)
    # biases: (nb_tracks, nb_states, nb_dims)
    C1 = current_hidden_var_coefs_1[:, :, :, coef_index:coef_index+1] + tf.random.normal([1,1,1,1], 0, 1e-20, dtype=dtype)
    C2 = current_hidden_var_coefs_2[:, :, :, coef_index:coef_index+1] + tf.random.normal([1,1,1,1], 0, 1e-20, dtype=dtype)
    ...
    C1_b = C1[..., 0]   # (nb_tracks, nb_states, nb_LocErr_dims) -> broadcasts on nb_dims
    C2_b = C2[..., 0]
    biases1 = tf.math.divide_no_nan(biases_1, C1_b)
    biases2 = tf.math.divide_no_nan(biases_2, C2_b)

    var1 = 1. / (C1**2 + tf.random.normal([1,1,1,1], 0, 1e-100, dtype=dtype))
    var2 = 1. / (C2**2 + tf.random.normal([1,1,1,1], 0, 1e-100, dtype=dtype))
    var3 = var1 + var2
    std3 = var3**0.5
    ...
    LogConstant = - tf.reduce_sum(tf.math.log(tf.math.abs(C1 * C2 * std4 * std3)[..., 0]), axis=-1)      # (nb_tracks, nb_states)
...
def final_RNN_function_phase_1(current_hidden_var_coefs, next_hidden_var_coefs, biases, coef_index, ID_1, ID_2, nb_hidden_variables, LC, nb_gaussians, kept_next_hidden_var_coefs, kept_biases):
    ...
    _c = current_hidden_var_coefs_cp[ID_2][:, :, :, coef_index]          # (nb_tracks, nb_states, nb_LocErr_dims)
    LC += - tf.reduce_sum(tf.math.log(tf.abs(_c)), axis=-1)
...
class Initial_layer_constraints(tf.keras.layers.Layer):
    ...
    blur_ratio             : Fration of the time step where the image is actually acquired.
                             This enables to alleviate the bias of the diffusion lengths
                             caused by motion blur. Set to 1 if the imaging is continuous,
                             or to the ratio exposure time over time between frame
                             in case of stroboscopic imaging.
    vary_params,
    vary_initial_params,
    ...
    def __init__(
        ...
        **kwargs):
        ...
        self.blur_ratio = tf.constant(blur_ratio, dtype = dtype)
        
        initial_sequence_phase_1, initial_sequence_phase_2, recurrent_sequence_phase_1, recurrent_sequence_phase_2, final_sequence_phase_1, transition_sequence = get_sequences(params, initial_params, constraint_function, nb_gaussians, nb_hidden_vars, blur_ratio, dtype)
    ...
    def build(self, input_shape):
        ...
        if self.carryover:
            self.carryout_coefs = tf.Variable(np.zeros((self.nb_hidden_vars, input_shape[2], nb_sequences, self.nb_dims, self.nb_hidden_vars)), dtype = dtype, trainable = False)
            self.carryout_biases = tf.Variable(np.zeros((self.nb_hidden_vars, input_shape[2], nb_sequences, self.nb_dims)), dtype = dtype, trainable = False)
    ...
    def call(self, inputs, input_LocErrs, input_dts):
        ...
        next_initial_hidden_var_coefs = tf.zeros((nb_hidden_vars, nb_tracks, nb_states, nb_dims, nb_hidden_vars), dtype = dtype)  # these coefs must equal 0 as the initial gaussians must only depend on the fist set of hidden states

        # Gaussians added at state transitions
        transition_hidden_var_coefs = transition_hidden_var_coefs/transition_Gaussian_stds
        transition_biases = transition_biases/transition_Gaussian_stds[..., 0, :]
        ...
        '''
        # observation fold: diagonal over the spatial axis (L broadcasts to nb_dims)
        biases += tf.math.reduce_sum(obs_var_coefs * inputs[0], axis = -1)
        initial_biases += tf.math.reduce_sum(initial_obs_var_coefs * inputs[0], axis = -1)
        ...
        # log-factor scaling: nb_dims already folded in constraint_function
        init_log_factors = tf.concat([initial_Log_factors]*sequence_length, axis = 1)
...
class Initial_layer_constraints_rotational(tf.keras.layers.Layer):
    '''
    First layer of the model that initializes the parameters and variables
    
    Same as Initial_layer_constraints in the basic exatrack script but builds the
    appended mislinking-state row for the 13-column (translational + rotational) 
    parameter layout.

    The mislinking state is a "junk" state that explains wrongly-linked
    detections: large translational diffusion, no anomalous structure, and a
    large (essentially uniform) rotational diffusion so orientation is
    unconstrained. Motion types are set to confined (0) with ano = -15, which
    reduces both channels to pure diffusion.
    ...
    def call(self, inputs, input_LocErrs, input_dts):
        ...
        # ---- Mislinking state row for the 13-column layout ----
        # [log_d, ano, log_q, is_dir, LE_x, LE_y, LE_z,
        #  rot_log_d, rot_ano, rot_log_q, rot_is_dir, OE_1, OE_2]
        mislink_row = tf.concat((
            [tf.math.log(tf.cast(max_linking_distance, dtype=dtype)),   # log_d  (large translational diffusion)
             tf.constant(-15., dtype=dtype),                            # ano    (no anomalous structure)
             tf.math.log(tf.cast(1e-5, dtype=dtype)),                   # log_q  (tiny)
             tf.constant(0.,  dtype=dtype)],                            # is_dir (confined)
            param_vars[-1, 4:7],                                        # LE_x, LE_y, LE_z (from last real state)
            [tf.math.log(tf.cast(np.pi, dtype=dtype)),                  # rot_log_d (large: orientation ~ uniform)
             tf.constant(-15., dtype=dtype),                            # rot_ano
             tf.math.log(tf.cast(1e-5, dtype=dtype)),                   # rot_log_q
             tf.constant(0.,  dtype=dtype)],                            # rot_is_dir (confined)
            param_vars[-1, 11:13],                                      # OE_1, OE_2 (from last real state)
        ), axis=0)
        ...
        (hidden_var_coefs, obs_var_coefs, Gaussian_stds, biases,
         initial_hidden_var_coefs, initial_obs_var_coefs, initial_Gaussian_stds,
         initial_biases, transition_hidden_var_coefs, transition_Gaussian_stds,
         transition_biases, integration_variable_index, Log_factors,
         initial_Log_factors, transition_Log_factors) = constraint_function(
            param_vars, initial_param_vars, input_LocErrs, input_dts,
            nb_dims, reference_dt, LocErr_function, blur_ratio, dtype)
        ...
        next_initial_hidden_var_coefs = tf.zeros(
            (nb_hidden_vars, nb_tracks, nb_states, nb_dims, nb_hidden_vars), dtype=dtype)
        ...
        initial_states = [Next_coefs, Next_biases, LP, Log_factors, transition_Log_factors,
...
@tf.function
def constraint_function(params, initial_params, LocErrs, dts,
                        nb_dims, reference_dt, LocErr_function,
                        blur_ratio=1, dtype='float64'):
    """
    Time-varying constraint function for the diffusion + rotational-diffusion
    model. Identical in spirit to the original anisotropic constraint_function;
    the ONLY difference is how the per-channel parameters are assembled:

        channels 0,1,2 (x,y,z)      <- translational params (cols 0..6)
        channels 3,4  (angle1,angle2) <- rotational params    (cols 7..12)

    `nb_dims` passed by the caller is overridden internally by
    NB_DIMS_TOTAL = 5, because the channel layout is fixed by this model.

    Everything downstream (Gaussian coefficients g0/g1/g2, biases, std-rescaling
    and log-factors) is computed per channel and summed over channels, so it is
    reused verbatim. See the module docstring for the parameter layout.
    ...
    nb_dims = NB_DIMS_TOTAL   # fixed: 3 translational + 2 rotational channels

    # ------------------------------------------------------------------
    # dts -> (track_len+1, nb_tracks, 1, 1)
    # ------------------------------------------------------------------
    dts = tf.cast(dts, dtype)
    dts = tf.transpose(dts, [1, 0])
    dts = dts[..., None, None]
    reference_dt = tf.cast(reference_dt, dtype)

    # LocErrs: keep the trailing channel axis
    LocErrs = tf.cast(LocErrs, dtype)
    if LocErrs.shape.rank == 2:
        LocErrs = LocErrs[..., None]                 # (nb_tracks, track_len, 1)
    LocErrs = tf.transpose(LocErrs, [1, 0, 2])       # (track_len, nb_tracks, L_in)
    LocErrs = LocErrs[:, :, None, :]                 # (track_len, nb_tracks, 1, L_in)

    track_len = tf.shape(LocErrs)[0]
    nb_tracks = tf.shape(LocErrs)[1]

    # ==================================================================
    # Per-CHANNEL parameter assembly (translational | rotational)
    # ==================================================================
    def _per_dim(trans_vec, rot_vec):
        """(nb_states,), (nb_states,) -> (1, 1, nb_states, nb_dims=5)."""
        t = tf.repeat(trans_vec[:, None], NB_TRANS_DIMS, axis=1)   # (S, 3)
        r = tf.repeat(rot_vec[:, None],   NB_ROT_DIMS,   axis=1)   # (S, 2)
        return tf.concat([t, r], axis=1)[None, None]               # (1,1,S,5)

    # translational per-state scalars (columns 0..3)
    trans_log_d, trans_ano, trans_log_q, trans_is_dir = (
        params[:, 0], params[:, 1], params[:, 2], params[:, 3])
    # rotational per-state scalars (columns 7..10)
    rot_log_d, rot_ano, rot_log_q, rot_is_dir = (
        params[:, 7], params[:, 8], params[:, 9], params[:, 10])

    log_d  = _per_dim(trans_log_d,  rot_log_d)
    ano    = _per_dim(trans_ano,    rot_ano)
    log_q  = _per_dim(trans_log_q,  rot_log_q)
    is_dir = _per_dim(trans_is_dir, rot_is_dir)

    # localization (cols 4..6) / orientation (cols 11..12) errors -> per channel
    LocErr_param = tf.math.exp(
        tf.concat([params[:, 4:7], params[:, 11:13]], axis=1))[None, None]  # (1,1,S,5)
    
    # initial spread: col 0 translational, col 1 rotational (fallback to col 0)
    trans_init_spread = initial_params[:, 0]
    if initial_params.shape[1] > 1:
        rot_init_spread = initial_params[:, 1]
    else:
        rot_init_spread = initial_params[:, 0]
    log_init_spread = _per_dim(trans_init_spread, rot_init_spread)
    
    # ------------------------------------------------------------------
    # Apply the input localization/orientation errors to the parameters
    # ------------------------------------------------------------------
    LocErrs = LocErr_function(LocErrs, LocErr_param)                       # (track_len,nb_tracks,S,·)
    LocErr_b = tf.broadcast_to(LocErrs + 1e-20,
                               (track_len, nb_tracks, nb_states, nb_dims))
    
    # State-selection masks, shape (1, 1, nb_states, nb_dims)
    ...
    zeros_s = tf.zeros((track_len, nb_tracks, nb_states, nb_dims), dtype=dtype)
    tiny    = tf.fill((track_len, nb_tracks, nb_states, nb_dims), tf.constant(1e-15, dtype=dtype))
    zeros_d = tf.zeros_like(LocErr_b)
    ...
    g0 = tf.stack([(1 - 0.5 * blur_ratio_eff) / LocErr_b, zeros_d,
                   0.5 * blur_ratio_eff / LocErr_b,       zeros_d], axis=-1)
    ...
    g2    = tf.stack([zeros_s, g2_c1, zeros_s, -inv_q], axis=-1)
    
    hidden_vars = tf.stack([g0, g1, g2], axis=1)                               # (t,3,n,s,L,4)
    
    # ==================================================================
    # Recurrent observation coefficients
    # ==================================================================
    obs_g0   = (-1.0 / LocErr_b)[..., None]
    obs_zero = zeros_d[..., None]
    obs_vars = tf.stack([obs_g0, obs_zero, obs_zero], axis=1)                  # (t,3,n,s,L,1)
    
    # ==================================================================
    # Initial hidden-variable coefficients
    # ==================================================================
    init_g0 = tf.stack([1.0 / initial_position_spread, zeros_s], axis=-1)[0]   # (n,s,L,2)
    
    init_g1_c0 = (1.0  / well_distance) * isconf_mask + tiny * isdir_mask
    init_g1_c1 = (-1.0 / well_distance) * isconf_mask + (1.0 / v) * isdir_mask
    init_g1    = tf.stack([init_g1_c0, init_g1_c1], axis=-1)                   # (t,n,s,L,2)
    initial_hidden_vars = tf.stack([init_g0, init_g1[0]], axis=0)             # (2,n,s,L,2)
    
    # ==================================================================
    # Transition hidden-variable coefficients
    # ==================================================================
    transition_hidden_vars = init_g1[:, None]                                 # (t,1,n,s,L,2)

    # ==================================================================
    # Unit-std / zero-bias scaffolding tensors
    # ==================================================================
    Gaussian_stds = tf.ones((track_len, nb_obs_vars + nb_hidden_vars,
                             nb_tracks, nb_states, nb_dims, 1), dtype=dtype)
    biases = tf.zeros((track_len, nb_obs_vars + nb_hidden_vars,
                       nb_tracks, nb_states, nb_dims), dtype=dtype)
    initial_obs_vars      = tf.zeros((nb_hidden_vars, nb_tracks, nb_states, nb_dims, nb_obs_vars), dtype=dtype)
    initial_Gaussian_stds = tf.ones((nb_hidden_vars, nb_tracks, nb_states, nb_dims, 1), dtype=dtype)
    initial_biases        = tf.zeros((nb_transition_gaussians, nb_tracks, nb_states, nb_dims), dtype=dtype)
    ...
    # Log factors summed over ALL 5 channels (nb_dims already folded in here).
    ...
    log_init_spread = tf.reduce_sum(log_init_spread, axis=-1)

    initial_Log_factors    = Log_factors[0] - log_init_spread[0] + initial_anomalous_factor[0]
...
def RNN_cell(input_i, Prev_coefs, Prev_biases, LP, segment_len, reshaped_Log_factors, reshaped_transition_Log_factors, reccurent_obs_var_coefs, reccurent_hidden_var_coefs, reccurent_next_hidden_var_coefs, reccurent_biases, transition_hidden_var_coefs, transition_biases, sequence_phase_1, sequence_phase_2, transition_mask, transition_sequence, transition_mean, transition_var, gamma_dist_mean, gamma_dist_var, states, dt_ratios):
    ...
    all_Prev_coefs = transition_Prev_coefs*transition_mask[None,:,:,None,None] + Prev_coefs2*(1-transition_mask[None,:,:,None,None])
    ...
    transition_probas = tf.clip_by_value(1 - S_new / S_old, clip_value_min=1e-20, clip_value_max=1 - 1e-10)
    ...
    reshaped_Next_coefs = tf.reshape(Next_coefs, Next_coefs.shape[:2]+[sequence_length*nb_states, nb_states, nb_dims, nb_hidden_vars])
    transition_LPs = tf.reshape(all_LP - 200*(1-transition_mask), (nb_tracks, sequence_length*nb_states, nb_states)) - tf.math.reduce_sum(tf.math.log(tf.math.abs(tf.linalg.diag_part(tf.transpose(reshaped_Next_coefs, [1, 2, 3, 4, 0, 5])))+1e-20), axis = (-2,-1))
    ...
    transition_Next_coefs = tf.reshape(Next_coefs, Next_coefs.shape[:2]+[sequence_length*nb_states, nb_states, nb_dims, nb_hidden_vars])
    transition_Next_coefs = tf.reduce_sum(transition_Next_coefs*transition_weights[None, :,:,:,None, None], axis = 2)
    ...
    transition_LPs = tf.math.log(tf.reduce_sum(transition_Ps, axis = 1)) + max_transition_LPs[:,0] + tf.reduce_sum(tf.math.log(tf.math.abs(tf.linalg.diag_part(tf.transpose(transition_Next_coefs, [1, 2, 3, 0, 4]))) + 1e-20), axis =  (-2,-1))
    ...
    stable_Next_coefs = tf.reduce_sum(tf.reshape(Next_coefs, Next_coefs.shape[:2]+[sequence_length*nb_states, nb_states, nb_dims, nb_hidden_vars])*stable_weights[None,:,:,:,None, None], axis = 3)
    ...
    last_Next_coefs = tf.reshape(Next_coefs[:, :, -nb_states*2:], (nb_prev_gaussians, nb_tracks, 2, nb_states, nb_dims, nb_hidden_vars))
    last_Next_biases = tf.reshape(Next_biases[:, :, -nb_states*2:], (nb_prev_gaussians, nb_tracks, 2, nb_states, nb_dims))
    last_LP = tf.reshape(new_LP[:, -nb_states*2:], (nb_tracks, 2, nb_states)) - tf.math.reduce_sum(tf.math.log(tf.math.abs(tf.linalg.diag_part(tf.transpose(last_Next_coefs, [1, 2, 3, 4, 0, 5]))) + 1e-20), axis=(-2, -1))
    ...
    reduced_last_Next_coefs = tf.reduce_sum(last_Next_coefs*last_weights[None,:,:,:,None,None], axis = 2)
    reduced_last_Next_biases = tf.reduce_sum(last_Next_biases*last_weights[None,:,:,:, None], axis = 2)
    reduced_last_LPs = (tf.math.log(sum_last_P + 1e-100) + last_LP_max)[:,0] + tf.math.reduce_sum(tf.math.log(tf.math.abs(tf.linalg.diag_part(tf.transpose(reduced_last_Next_coefs, [1, 2, 3, 0, 4])))+1e-20), axis = (-2, -1))
...
class Custom_RNN_layer(tf.keras.layers.Layer):
    ...
    def call(self, inputs, input_dts, reference_dt, mask,
        ...
        def body(i, Prev_coefs, Prev_biases, LP, segment_len,
            ...
            log_w = LP - tf.math.reduce_sum(
                tf.math.log(tf.math.abs(tf.reduce_prod(tf.linalg.diag_part(tf.transpose(Prev_coefs, [1, 2, 3, 0, 4])), axis=-1)
                                        ) + 1e-20),
                axis = -1)
            ...
            mask_coef   = mask_i[None, :, None, None, None]
            mask_bias   = mask_i[None, :, None, None]
            mask_scalar = mask_i[:, None]
            mask_state  = mask_i[:, None, None, None]
            
            Prev_coefs      = Next_coefs       * mask_coef   + Prev_coefs      * (1 - mask_coef)
            Prev_biases     = Next_biases      * mask_bias   + Prev_biases     * (1 - mask_bias)
...
def marginalise_variable(All_coefs, All_biases, integrate_index):
    """
    Integrates (marginalises) over the hidden variable `integrate_index` from
    the two Gaussians (G = 2) that jointly describe (pos, ano), INDEPENDENTLY
    per spatial dimension.

    Expected shapes
    ---------------
    All_coefs  : (..., D, G, H)   e.g. (N, T, Q, D, G, H), with G = 2, H = 2.
    All_biases : (..., D, G)      e.g. (N, T, Q, D, G).      <-- G now LAST

    Returns
    -------
    coefs3  : (..., D, H)  marginal (integrate-independent) Gaussian coefs, per dim.
    biases3 : (..., D)     its bias, per dim.
    coefs4  : (..., D, H)  coefs of the Gaussian that carried the integrated var.
    biases4 : (..., D)     its bias.
    LogConstant : (...)    change-of-variables constant, summed over D.
    """
    keep_index = 1 - integrate_index

    # per-gaussian, per-dimension slices
    coefs_g0 = All_coefs[..., 0, :]                 # (..., D, H)
    coefs_g1 = All_coefs[..., 1, :]                 # (..., D, H)
    C1 = All_coefs[..., 0, integrate_index]         # (..., D)  coef of int-var, G0
    C2 = All_coefs[..., 1, integrate_index]         # (..., D)  coef of int-var, G1

    biases_g0 = All_biases[..., 0]                  # (..., D)  G is now the LAST axis
    biases_g1 = All_biases[..., 1]                  # (..., D)

    # keepdim on H so the scalar-per-dim coef broadcasts against the H vector
    C1_h = C1[..., None]                            # (..., D, 1)
    C2_h = C2[..., None]                            # (..., D, 1)

    # normalise each Gaussian by its integrated-variable coefficient
    coefs1  = coefs_g0 / (C1_h + 1e-30)             # (..., D, H)
    coefs2  = coefs_g1 / (C2_h + 1e-30)
    biases1 = biases_g0 / (C1 + 1e-30)              # (..., D)  per-dim division
    biases2 = biases_g1 / (C2 + 1e-30)

    var1 = 1.0 / (C1_h**2 + 1e-30)                  # (..., D, 1)
    var2 = 1.0 / (C2_h**2 + 1e-30)
    var3 = var1 + var2
    std3 = np.sqrt(var3)                            # (..., D, 1)
    std3_d = std3[..., 0]                           # (..., D)

    coefs3  = (coefs1 - coefs2) / std3              # (..., D, H)
    biases3 = (biases1 - biases2) / std3_d          # (..., D)

    var4 = var1 * var2 / var3
    std4 = var4**0.5                                # (..., D, 1)
    std4_d = std4[..., 0]
    var1_d, var2_d, var3_d = var1[..., 0], var2[..., 0], var3[..., 0]

    coefs4  = (coefs1 * var2 + coefs2 * var1) / (var3 * std4)             # (..., D, H)
    biases4 = (biases1 * var2_d + biases2 * var1_d) / (var3_d * std4_d)   # (..., D)

    # per-dimension log-constant summed over the spatial axis
    LogConstant = -np.sum(np.log(np.abs(C1 * C2 * std4_d * std3_d)), axis=-1)  # (...)
...
def extract_hidden_variables(All_coefs, All_biases, All_LPs, nb_dims, sequence_length):
    ...
    Expected shapes
    ---------------
    All_coefs  : (N, T, Q, D, G, H)   G = 2, H = 2, Q = sequence_length*nb_states
    All_biases : (N, T, Q, D, G)      
    All_LPs    : (N, T, Q)

    Returns (signature unchanged)
    -----------------------------
    pos_mean       : (N, T, D)
    anomalous_mean : (N, T, nb_states, D)
    position_std   : (N, T, D)
    anomalous_std  : (N, T, nb_states, D)
    '''
    ...
    # =========================== Anomalous variable =====================
    integrate_index = 0                      # integrate out position -> keep ano
    keep_index = 1 - integrate_index         # = 1
    coefs3, biases3, coefs4, biases4, LogConstant = marginalise_variable(
        All_coefs, All_biases, integrate_index)

    # coefs4[..., integrate_index] : (N,T,Q,D) -> sum log over D (was nb_dims*log)
    All_LPs_ano = (All_LPs + LogConstant
                   - np.sum(np.log(np.abs(coefs4[..., integrate_index]) + 1e-30), axis=-1))
    All_LPs_ano = All_LPs_ano.reshape((nb_tracks, track_len, sequence_length, nb_states))

    # per-dimension MAP / variance of the anomalous variable
    ano_MAP = -biases3 / coefs3[..., keep_index]                          # (N,T,Q,D)
    ano_MAP = ano_MAP.reshape((nb_tracks, track_len, sequence_length, nb_states, nb_dims))
    ano_var = 1.0 / (coefs3[..., keep_index]**2 + 1e-50)                  # (N,T,Q,D)
    ano_var = ano_var.reshape((nb_tracks, track_len, sequence_length, nb_states, nb_dims))

    w_ano = scipy_softmax(All_LPs_ano, axis=2)[..., None]                 # (N,T,seq,nb_states,1)
    weighted_ano_MAP = ano_MAP * w_ano
    anomalous_mean = np.sum(weighted_ano_MAP, axis=2)                     # (N,T,nb_states,D)
    weighted_ano_var_term1 = (ano_var + ano_MAP**2) * w_ano
    weighted_ano_var_term2 = ano_MAP * w_ano
    anomalous_var = (np.sum(weighted_ano_var_term1, axis=2)
                     - np.sum(weighted_ano_var_term2, axis=2)**2)
    anomalous_std = anomalous_var**0.5

    # =============================== Position ===========================
    integrate_index = 1                      # integrate out ano -> keep position
    # normalise by the integrated (ano) coef of gaussian 1, per dim, summed over D
    All_LPs_pos = (All_LPs
                   - np.sum(np.log(np.abs(All_coefs[..., integrate_index, integrate_index]) + 1e-30),
                            axis=-1))

    pos_coef0 = All_coefs[..., 0, 0]                                     # (N,T,Q,D)  G0, pos var, per dim
    pos_MAP = -All_biases[..., 0] / pos_coef0                           # (N,T,Q,D)  G0 bias (G last)
    pos_var = 1.0 / (pos_coef0**2 + 1e-50)                              # (N,T,Q,D)

    w_pos = scipy_softmax(All_LPs_pos, axis=2)[..., None]               # (N,T,Q,1)  softmax over Q
    weighted_pos_MAP = pos_MAP * w_pos
    pos_mean = np.sum(weighted_pos_MAP, axis=2)                         # (N,T,D)
    weighted_pos_var_term1 = (pos_var + pos_MAP**2) * w_pos
    weighted_pos_var_term2 = pos_MAP * w_pos
    position_var = (np.sum(weighted_pos_var_term1, axis=2)
                    - np.sum(weighted_pos_var_term2, axis=2)**2)
...
def get_sequences(params, initial_params, constraint_function, nb_gaussians, nb_hidden_vars, blur_ratio, dtype):
    '''
    ...
    def LocErr_function(LocErrs, LocErr_param):
        return LocErrs
    dts = np.ones((1,2))
    hidden_var_coefs, _, _, _, initial_hidden_var_coefs, _, _, _,  transition_hidden_var_coefs, _, _, integration_variable_index, _, _, _ = constraint_function(params, initial_params, LocErrs, dts, nb_dims, 1., LocErr_function, blur_ratio, dtype)
    hidden_var_coefs = hidden_var_coefs[0]
    transition_hidden_var_coefs = transition_hidden_var_coefs[0]
    
    recurrent_current_hidden_var_coefs = np.copy(hidden_var_coefs[:,0,0,0,:nb_hidden_vars])
    recurrent_next_hidden_var_coefs = np.copy(hidden_var_coefs[:,0,0,0,nb_hidden_vars:])
    current_hidden_var_coefs = hidden_var_coefs[:,0,0,0,:nb_hidden_vars]
    next_hidden_var_coefs = hidden_var_coefs[:,0,0,0,nb_hidden_vars:]
    
    current_initial_hidden_var_coefs = initial_hidden_var_coefs[:,0,0,0,:nb_hidden_vars]
    ...
    current_hidden_var_coefs = np.concatenate((current_hidden_var_coefs, transition_hidden_var_coefs[:, 0,0,0]), 0)
    next_hidden_var_coefs = np.concatenate((next_hidden_var_coefs, transition_hidden_var_coefs[:, 0,0,0]*0), 0)
...
def build_segment_model(segment_length,
                        nb_states,
                        params,                    # (nb_states, 13)   see module docstring
                        initial_params,            # (nb_states, >=2)  [trans spread, rot spread]
                        transition_rates,
                        transition_shapes,
                        initial_fractions,
                        batch_size,
                        reference_dt,
                        nb_dims=NB_DIMS_TOTAL,     # = 5 : [x, y, z, angle1, angle2]
                        sequence_length=3,
                        ...
                        nb_LocErr_dims=0,          # 5 for anisotropic (3 loc + 2 orient) errors; 0 for a scalar/Constant error
                        blur_ratio=0.00001,
                        LocErr_type='Linear'):
    """
    Build the diffusion + rotational-diffusion segmented model.

    The input tracks must have nb_dims = 5 columns: [x, y, z, angle1, angle2].
    If per-point measurement errors are supplied (nb_LocErr_dims=5) they must
    also have 5 columns: 3 localization errors + 2 orientation errors, in the
    same channel order. With nb_LocErr_dims=0 a single input-error column (or
    none) is broadcast to all channels; combine that with LocErr_type='Constant'
    to fit purely from the per-state error parameters.
    """
    nb_obs_vars = 1
    nb_hidden_vars = 2
    nb_gaussians = nb_obs_vars + nb_hidden_vars
    
    inputs = tf.keras.Input(batch_shape=(batch_size, segment_length, nb_dims), name='tracks', dtype=dtype)
    if nb_LocErr_dims > 0:
        input_LocErrs = tf.keras.Input(batch_shape=(batch_size, segment_length, nb_LocErr_dims),
                                       name='Localization errors', dtype=dtype)
    else:
        input_LocErrs = tf.keras.Input(batch_shape=(batch_size, segment_length),
                                       name='Localization errors', dtype=dtype)
    input_dts = tf.keras.Input(batch_shape=(batch_size, segment_length + 1), name='frame durations', dtype=dtype)
    input_mask = tf.keras.Input(batch_shape=(batch_size, segment_length), name='masks', dtype=dtype)
    ...
    Init_layer = Initial_layer_constraints_rotational(
        nb_states, nb_gaussians, nb_obs_vars, nb_hidden_vars, nb_dims,
        params, initial_params, initial_fractions, max_linking_distance,
        constraint_function, reference_dt=reference_dt,
        vary_params=vary_params, vary_initial_params=vary_initial_params,
        vary_initial_fractions=vary_initial_fractions, sequence_length=sequence_length,
        carryover=True, LocErr_type=LocErr_type, blur_ratio=blur_ratio, dtype=dtype)
    ...
    log_ds            = Init_layer.param_vars[:, 0]   # translational log diffusion length
    anomalous_factors = Init_layer.param_vars[:, 1]   # translational anomalous parameter
    isdir             = Init_layer.param_vars[:, 3]   # translational motion type  (was col -1 in old layout)
    ...
    Prev_coefs = first_mask_layer(Prev_coefs, Init_layer.carryout_coefs, input_isfirst[None, :, None, None, None])
    ...
    layer = Custom_RNN_layer(
        ...
        sequence_length=sequence_length, vary_transition_shapes=vary_transition_shapes,
        vary_transition_rates=vary_transition_rates, carryover=True, dtype=dtype)
    ...
    carryover_layer = CarryoverAssignLayer(
        carryout_variables=[Init_layer.carryout_coefs, Init_layer.carryout_biases,
                            Init_layer.carryout_LP, layer.carryout_segment_len,
                            layer.carryout_gamma_dist_mean, layer.carryout_gamma_dist_var],
        dtype=dtype)

    F_layer = Final_layer(Init_layer.final_sequence_phase_1, nb_dims=nb_dims,
    ...
    model = tf.keras.Model(inputs=(inputs, input_LocErrs, input_dts, input_mask, input_isfirst),
                           outputs=outputs, name="Diffusion_rotation_model")
    pred_model = tf.keras.Model(inputs=(inputs, input_LocErrs, input_dts, input_mask, input_isfirst),
                                ...
                                name="Diffusion_rotation_model")
    return model, pred_model

class get_parameters(tf.keras.callbacks.Callback):
    """
    Keras callback that snapshots the fitted parameters at the start of training
    and at the end of every epoch, for the 13-column (translational +
    rotational) layout. Use it to follow convergence of every physical
    parameter, per state, over the fit.
 
        cb = get_parameters(track_segmentation=True)
        model.fit(..., callbacks=[cb])
 
        cb.raw_history     # list of (nb_states, 13) arrays, one per snapshot
        cb.param_history   # list of get_model_params() dicts, one per snapshot
        cb.loss_history    # list of training losses (one per epoch)
        cb.history_of('d') # -> np.array of shape (nb_snapshots, nb_states)
 
    `raw_history` keeps the untouched 13-column weight matrix (so nothing is
    ever lost regardless of column meaning); `param_history` keeps the parsed,
    ...
    def _snapshot(self):
        raw = self.model.weights[0].numpy().copy()                 # (nb_states, 13)
        parsed = get_model_params(self.model)
        self.raw_history.append(raw)
        self.param_history.append(parsed)
        return parsed
    ...
    def on_epoch_end(self, epoch, logs=None):
        parsed = self._snapshot()
        if logs is not None and 'loss' in logs:
            self.loss_history.append(float(logs['loss']))
        if self.verbose:
            with np.printoptions(precision=4, suppress=True):
                print(f"d={parsed['d']}  ano={parsed['Translational anomalous factors']}  q={parsed['q']}  "
                      f" Loc Error={parsed['Localization errors']} rot_d={parsed['rot_d']}  Rot Error={parsed['Orientation errors']} rot_ano={parsed['Rotational anomalous factors']} rot_q={parsed['rot_q']} rates={parsed['transition rates']}")
 
    def history_of(self, key):
        """Stack one parsed key across all snapshots -> (nb_snapshots, ...)."""
        return np.array([snap[key] for snap in self.param_history])
 

def get_model_params(model):
    """Return a dict of physical parameters for the 13-column layout,
    with separate translational and rotational blocks."""
    ...
    p = weights[0]
    trans_types = p[:, 3].numpy().astype(bool).astype(int)
    rot_types   = p[:, 10].numpy().astype(bool).astype(int)
 
    trans_ano = (tf.sigmoid(p[:, 1]) * (1 - p[:, 3]) + 3 ** 0.5 * tf.exp(p[:, 1]) * p[:, 3]).numpy()
    rot_ano   = (tf.sigmoid(p[:, 8]) * (1 - p[:, 10]) + 2 ** 0.5 * tf.exp(p[:, 8]) * p[:, 10]).numpy()
    
    return {
        'Translational model types': np.array(['Confined motion', 'Directed motion'])[trans_types],
        'Rotational model types':    np.array(['Confined rotation', 'Directed rotation'])[rot_types],
        'Translational anomalous factors': trans_ano,
        'Rotational anomalous factors':    rot_ano,
        'd':      np.exp(p[:, 0].numpy()),           # translational diffusion length
        'q':      np.exp(p[:, 2].numpy()),           # translational anomalous variation speed
        'Localization errors':  np.exp(p[:, 4:7].numpy()),   # (nb_states, 3)
        'rot_d':  np.exp(p[:, 7].numpy()),           # rotational diffusion length
        'rot_q':  np.exp(p[:, 9].numpy()),           # rotational anomalous variation speed
        'Orientation errors':   np.exp(p[:, 11:13].numpy()), # (nb_states, 2)
...
def model_to_DataFrame(model, state_names=None):
    """
    Turn a fitted diffusion + rotational-diffusion model into a tidy pandas
    DataFrame with one row per state, for the 13-column layout.
 
    Columns
    -------
    state, [fraction],
    trans_type, d, trans_anomalous, q, LocErr_x, LocErr_y, LocErr_z,
    rot_type, rot_d, rot_anomalous, rot_q, OrientErr_1, OrientErr_2
 
    The (nb_states x nb_states) transition-rate and transition-shape matrices are
    attached as df.attrs['transition_rates'] / df.attrs['transition_shapes'],
    since they are per-transition rather than per-state.
    """
    import pandas as pd
 
    p = get_model_params(model)
    nb_states = len(p['d'])
    LE = np.asarray(p['Localization errors'])   # (nb_states, 3)
    OE = np.asarray(p['Orientation errors'])    # (nb_states, 2)
 
    if state_names is None:
        state_names = [f'state_{i}' for i in range(nb_states)]
 
    df = pd.DataFrame({
        'state':          state_names,
        # ---- translational block ----
        'trans_type':      p['Translational model types'],
        'd':               p['d'],
        'trans_anomalous': p['Translational anomalous factors'],
        'q':               p['q'],
        'LocErr_x':        LE[:, 0],
        'LocErr_y':        LE[:, 1],
        'LocErr_z':        LE[:, 2],
        # ---- rotational block ----
        'rot_type':        p['Rotational model types'],
        'rot_d':           p['rot_d'],
        'rot_anomalous':   p['Rotational anomalous factors'],
        'rot_q':           p['rot_q'],
        'OrientErr_1':     OE[:, 0],
        'OrientErr_2':     OE[:, 1],
    })
 
    # Fractions may or may not include the appended mislinking state; only add
    # them as a column when the length matches the real states, otherwise keep
    # them alongside the transition matrices in attrs.
    frac = np.asarray(p['Fractions']).ravel()
    if frac.shape[0] == nb_states:
        df.insert(1, 'fraction', frac)
    else:
        df.attrs['fractions'] = frac
 
    df.attrs['transition_rates']  = p['transition rates']
    df.attrs['transition_shapes'] = p['transition shapes']
    return df
...
def bootstrapping(model, 
    ...
    for i in range(bootstrap_number):
        ...
        parameter_dict = get_model_params(model)
