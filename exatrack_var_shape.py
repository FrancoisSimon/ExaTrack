# -*- coding: utf-8 -*-
"""
Created on Wed May 20 12:52:03 2026
...
class Initial_layer_constraints(tf.keras.layers.Layer):
    ...
    def call(self, inputs, input_LocErrs, input_dts):
        ...
        initial_fractions = tf.math.softmax(self.initial_fractions)+1e-6
...
def RNN_cell(input_i, Prev_coefs, Prev_biases, LP, segment_len, reshaped_Log_factors, reshaped_transition_Log_factors, reccurent_obs_var_coefs, reccurent_hidden_var_coefs, reccurent_next_hidden_var_coefs, reccurent_biases, transition_hidden_var_coefs, transition_biases, sequence_phase_1, sequence_phase_2, transition_mask, transition_sequence, transition_mean, transition_var, gamma_dist_mean, gamma_dist_var, states, dt_ratios):
    ...
    dist     = tf.compat.v1.distributions.Gamma(current_shapes, current_rates)
    dtau     = dt_ratios[:, None]                            # (nb_tracks, 1), ref_dt units
    
    # Hazard rate evaluated at midpoint of the step, times dtau.
    # For fixed dt=1 this is identical to the previously working formula.
    # `prob` is what gives the shape parameter a reliable closed-form gradient.
    mid               = segment_len + dtau / 2.0
    hazard            = (dist.prob(mid) + 1e-14) / (1.0 - dist.cdf(mid) + 1e-12)
    transition_probas = hazard * dtau
    '''
    # Exact conditional probability of a transition during this step:
    #   P = 1 - S(seg + dtau) / S(seg),   with seg already in ref_dt units.
    log_S_now  = tf.math.log1p(-dist.cdf(segment_len)  + 1e-15)
    log_S_next = tf.math.log1p(-dist.cdf(segment_len + dtau) + 1e-15)
    transition_probas = 1.0 - tf.exp(log_S_next - log_S_now)
    np.any(np.isnan(transition_probas))
    np.where(np.isnan(transition_probas))
    transition_probas[1, -9]
    (log_S_next - log_S_now)[1, -9]
    log_S_next[1, -9]
    log_S_now[1, -9]
    -dist.cdf(segment_len)[1, -9]
    -dist.cdf(segment_len + dtau)[1, -9]
    np.any(np.isnan(tf.exp(log_S_next - log_S_now)))
    transition_probas[1, -9]
    '''
    non_transition_probas = tf.repeat(1-tf.reduce_sum(tf.reshape(transition_probas*transition_mask, shape = (nb_tracks, nb_states*sequence_length, nb_states)), axis = 2), nb_states, axis = 1) # this will be useful when we focus on the non-transitioning sequences
    
    transition_probas = transition_probas*transition_mask + non_transition_probas*(1-transition_mask)
    transition_probas = tf.clip_by_value(transition_probas, clip_value_min=1e-20, clip_value_max=1.0 - 1e-10)
    # Then, we update the log probability
    all_LP = LP2 + tf.math.log(transition_probas) # + reshaped_transition_Log_factors
    #np.any(np.isnan(tf.math.log(transition_probas)))
    '''
    ...
    current_segment_len = tf.concat([tf.zeros((nb_tracks, nb_states), dtype = dtype)+0.01, stable_segment_len + additional_stable_segment_len], axis = 1)
...
class Custom_RNN_layer(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        if self.carryover:
            ...
            self.carryout_gamma_dist_mean = tf.Variable(np.ones((nb_tracks, sequence_length * nb_states**2)), dtype=dtype, name = 'carryover_gamma_dist_mean', trainable = False)
            self.carryout_gamma_dist_var = tf.Variable(np.ones((nb_tracks, sequence_length * nb_states**2)), dtype=dtype, name = 'carryover_gamma_dist_var', trainable = False)
...
def transition_param_function(transition_shapes, transition_rates, density, Fs,effective_ds,  dts, reference_dt, dtype):
    ...
    transition_shapes = tf.math.exp(transition_shapes) + 1
    ...
    #transition_rates = transition_rates[None,None]*transition_shapes[None,None]*dts[..., None, None]/reference_dt+1e-20
    transition_rates = transition_rates * transition_shapes + 1e-20             # (S, S)
    transition_rates = tf.broadcast_to(transition_rates[None, None], (nb_time_points, nb_tracks, nb_states, nb_states))
...
class get_parameters(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if self.track_segmentation:
            ...
            shape_idx = 5
            rate_idx = 4
        transition_shapes = tf.math.exp(weights[shape_idx]) + 1
        #transition_rates = tf.math.softmax(weights[rate_idx], axis = 1)*transition_shapes
        max_off_rate = 1/min_lifetime
        transition_rates = weights[rate_idx]
        #transition_rates = (tf.eye(nb_states, dtype = dtype) * (1 - max_off_rate + max_off_rate*tf.math.softmax(transition_rates, axis = 1)*transition_shapes) 
        #                   + (1-tf.eye(nb_states, dtype = dtype)) * ( max_off_rate*tf.math.softmax(transition_rates, axis = 1)*transition_shapes) + 1e-7)
        transition_rates = (tf.eye(nb_states, dtype = dtype) * (1 - max_off_rate + max_off_rate*tf.math.softmax(transition_rates, axis = 1)) 
                           + (1-tf.eye(nb_states, dtype = dtype)) * ( max_off_rate*tf.math.softmax(transition_rates, axis = 1)) + 1e-7)
...
def get_model_params(model, track_segmentation = False):
    ...
    if track_segmentation:
        ...
        shape_IDs = 5
        rates_IDs = 4    
    transition_shapes = tf.math.exp(weights[shape_IDs]).numpy()+1
