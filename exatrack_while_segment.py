...
class Custom_RNN_layer(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.transition_shapes = tf.Variable(transition_shapes, dtype = dtype, name = 'Transition shape', trainable = True, constraint=lambda w: tf.where(tf.greater_equal(w, 0), w, 0))
    ...
    def call(self, inputs, mask, Prev_coefs, Prev_biases, LP, Log_factors, transition_Log_factors,
        ...
        # ---- run the loop ----
        _, Prev_coefs, Prev_biases, LP, segment_len, gamma_dist_mean, gamma_dist_var, states, All_states_ta = tf.while_loop(
            cond,
            body,
            loop_vars=[
                tf.constant(0),
        ...
        return Prev_coefs, Prev_biases, LP, segment_len, gamma_dist_mean, gamma_dist_var, All_states, states
...
def build_model(track_len, # maximum number of time points in the input tracks 
    ...
    Prev_coefs, Prev_biases, LP, segment_len, gamma_dist_mean, gamma_dist_var, All_states, states = layer(sliced_inputs, sliced_mask, Prev_coefs, Prev_biases, LP, Log_factors, transition_Log_factors, reccurent_obs_var_coefs, reccurent_hidden_var_coefs, reccurent_next_hidden_var_coefs, reccurent_biases, transition_hidden_var_coefs, transition_biases, log_ds, softmax_inv_Fractions, anomalous_factors, isdir)
...
def build_segment_model(track_len, # maximum number of time points in the input tracks 
    ...
    #self=layer
    Prev_coefs, Prev_biases, LP, segment_len, gamma_dist_mean, gamma_dist_var, All_motion_states, motion_states = layer(sliced_inputs, sliced_mask, Prev_coefs, Prev_biases, LP, Log_factors, transition_Log_factors, reccurent_obs_var_coefs, reccurent_hidden_var_coefs, reccurent_next_hidden_var_coefs, reccurent_biases, transition_hidden_var_coefs, transition_biases, log_ds, softmax_inv_Fractions, anomalous_factors, isdir, isfirst = input_isfirst)
    ...
    pred_model = tf.keras.Model(inputs=(inputs, input_mask, input_isfirst),
                                outputs=All_states, name="Diffusion_model")
