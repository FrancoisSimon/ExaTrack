...
def anomalous_diff_transition(max_track_len=100,
    ...
    for k in range(nb_tracks):
        ...
        if dt_std>0:
            scale = dt_std**2 / dt
            shape = dt / scale
            dts = np.random.gamma(shape, scale, (track_len))
        else:
            dts = dt
...
def anomalous_diff_transition(max_track_len=100,
    ...
    if not np.all(np.array([len(Fs), len(Ds), len(velocities), len(angular_Ds),
        ...
        raise ValueError('Fs, Ds, velocities, angular_Ds, conf_forces, conf_Ds, conf_dists '
                         'and transition_matrix must all be arrays of the same length '
    ...
    CPD_transition_mat = np.zeros((nb_states, nb_states,
                                   nb_burning_steps + max_track_len * nb_sub_steps * 10))
    for state0 in range(nb_states):
        for state1 in range(nb_states):
            if state0 != state1:
                p = gamma.cdf(np.arange(nb_burning_steps + max_track_len * nb_sub_steps * 10),
                              shape_matrix[state0, state1],
                              scale=1/transition_matrix[state0, state1] * nb_sub_steps)
                p[-1] = 1
                CPD_transition_mat[state0, state1] = p
            else:
                CPD_transition_mat[state0, state1, -1] = 1
    ...
    '''
    When tracks appear in the field of view, they have already experienced time, therefor the
    first lifetime is not gamma distributed (except if shape = 1), if the shape is very high
    for instance the initial distribution will be uniform between 1 and the mean lifetime.
    The initial lifetime L=l follows a distribution that depends on the actual gamma-distributed
    lifetime G=g and the time the particle has already been in the current state prior the first
    step F=f.
    l+f=g
    P(f) = sum over f of P(g)P(g-f|g) with P(g-f|g) a uniform distribution from 1 to g
    '''
    ...
    for k in range(nb_tracks):
        ...
        # ---------------------------------------------------------------
        # Variable time steps: draw one dt per actual time step from a
        # gamma distribution with mean `dt` and std `dt_std`.
        # All nb_sub_steps sub-steps within the same time step share the
        # same per-sub-step dt = dts[i] / nb_sub_steps.
        # ---------------------------------------------------------------
        if dt_std > 0:
        ...
        while len(track) < track_len * nb_sub_steps:
            ...
            # The state-segment of length `l` (in sub-steps) can span several
            # actual time steps, each with its own dt. Break it into chunks
            # that lie within a single time step so dt is constant per call.
            remaining = l
            while remaining > 0:
                sub_idx = len(track)                      # next sub-step to fill
                ts_idx = sub_idx // nb_sub_steps          # which time step it belongs to
                sub_in_ts = sub_idx % nb_sub_steps        # position within that time step
                chunk_len = min(remaining, nb_sub_steps - sub_in_ts)
                chunk_sub_dt = dts[ts_idx] / nb_sub_steps  # per-sub-step dt for this chunk
                
                if nb_dims < 3:
                    segment = anomalous_diff_2D(track_len=chunk_len + 1,
                                                LocErr=0,
                                                D=D,
                                                velocity=velocity / nb_sub_steps,
                                                angular_D=angular_D,
                    ...
                    segment = anomalous_diff_3D(track_len=chunk_len + 1,
                                                LocErr=0,
                                                D=D,
                                                velocity=velocity / nb_sub_steps,
                                                angular_D=angular_D,
...
def constraint_function(all_params, all_initial_params, LocErr, dts,
    ...
    The Python-level ``for k in range(nb_states)`` loop of the original
    implementation has been replaced by slicing + masking so that every
    quantity is computed once for all states simultaneously.  The function
    now also accepts per-track ``LocErr`` and ``dts`` tensors, so that the
    same routine can be called from the initial layer (first time step) and
    from the recurrent layer (subsequent steps) with the values that apply
    ...
    all_params : tensor, shape (nb_states, 5)
        ...
        The last column is ~0 for confined motion and >=0.5 for directed.
    all_initial_params : tensor, shape (nb_states, >=1)
        Column 0 is log(initial_position_spread).
    LocErr : tensor
        Localisation error for the current step.  Accepted shapes:
        (nb_tracks,), (nb_tracks, 1), or (nb_tracks, nb_dims)
        (a trailing dim axis is averaged out so LocErr becomes scalar/track).
    dts : tensor
        Duration of the current step.  Accepted shapes: (nb_tracks,)
        or (nb_tracks, 1).
    nb_dims : int
        Number of spatial dimensions.
    reference_dt : scalar float or 0-D tensor
        Reference time step used to normalise model parameters.
    dtype : str
        TensorFlow dtype (e.g. 'float64').

    Returns
    -------
    Twelve-tuple, identical in order to the original constraint_function.
    Every coefficient tensor now has shape
        (nb_gaussians, nb_tracks, nb_states, nb_vars_last)
    i.e. the ``nb_tracks`` axis is already populated — callers therefore no
    longer need to apply ``tf.repeat(..., nb_tracks, axis=1)``.
    """
    ...
    # ------------------------------------------------------------------
    # Normalise LocErr and dts to shape (nb_tracks, 1)
    # ------------------------------------------------------------------
    LocErr = tf.cast(LocErr, dtype)
    LocErr = tf.reshape(LocErr, [tf.shape(LocErr)[0], -1])   # (nb_tracks, *)
    LocErr = tf.reduce_mean(LocErr, axis=-1, keepdims=True)  # (nb_tracks, 1)

    dts = tf.cast(dts, dtype)
    dts = tf.reshape(dts, [tf.shape(dts)[0], -1])
    dts = tf.reduce_mean(dts, axis=-1, keepdims=True)        # (nb_tracks, 1)

    reference_dt = tf.cast(reference_dt, dtype)
    nb_tracks    = tf.shape(LocErr)[0]
    
    # ------------------------------------------------------------------
    # Per-state parameters, broadcast-ready on (1, nb_states)
    # ------------------------------------------------------------------
    log_d           = all_params[:, 1][None, :]
    ano             = all_params[:, 2][None, :]
    log_q           = all_params[:, 3][None, :]
    is_dir          = all_params[:, 4][None, :]
    log_init_spread = all_initial_params[:, 0][None, :]
    
    # State-selection masks, shape (1, nb_states)
    isdir_mask  = tf.cast(is_dir >= 0.5, dtype)
    isconf_mask = 1.0 - isdir_mask

    # ------------------------------------------------------------------
    # Step-size scaling from reference_dt to the actual dts
    #   diffusion-like quantities (std)  -> multiplicative sqrt(dt/ref_dt)
    #   rate-like quantities             -> multiplicative  (dt/ref_dt)
    # At dt == reference_dt the formulas reduce exactly to those of the
    # original constraint_function.
    # ------------------------------------------------------------------
    dt_ratio      = dts / reference_dt                       # (nb_tracks, 1)
    dt_sqrt_ratio = tf.sqrt(dt_ratio)                        # (nb_tracks, 1)
    
    # Values at reference_dt (same expressions as in the original code)
    d_ref = tf.exp(log_d)           # (1, nb_states)
    q_ref = tf.exp(log_q)
    l_ref = tf.math.sigmoid(ano)  
    v_ref = tf.exp(ano)   
    
    # Here l_ref is discrete but to convert it to the the dts we need to turn 
    # it to its continuous form l_ref_c, multiply it by the dt ratio and discetize it
    # contiuous-discete formula: ld = 1 - tf.math.exp(-lc) or lc = -tf.math.log(1 - ld)
    
    # Values at the actual dt, shape (nb_tracks, nb_states)
    d = d_ref * dt_sqrt_ratio + 1e-20
    q = q_ref * dt_sqrt_ratio + 1e-20
    l_ref_c = -tf.math.log(1 - l_ref)   # Continuous form
    l_c = l_ref_c * dt_ratio            # multiplication
    l = 1 - tf.math.exp(-l_c) + 1e-20   # discretization
    v = v_ref * dt_ratio + 1e-20
    
    # Characteristic well distance for confined motion
    well_distance = d / tf.sqrt(2.0 * l)       # (nb_tracks, nb_states)

    # Initial position spread, broadcast to (nb_tracks, nb_states)
    initial_position_spread = tf.broadcast_to(tf.exp(log_init_spread),
                                              tf.shape(d))

    # LocErr broadcast across states
    LocErr_b = tf.broadcast_to(LocErr, tf.shape(d)) + 1e-20   # (nb_tracks, nb_states)
    
    zeros = tf.zeros_like(d)
    tiny  = tf.fill(tf.shape(d), tf.constant(1e-15, dtype=dtype))
    
    # ==================================================================
    # Recurrent hidden-variable coefficients
    # Shape: (3, nb_tracks, nb_states, 4)
    ...
    g1_c1  = (l * isconf_mask + isdir_mask) * inv_d 
    ...
    hidden_vars = tf.stack([g0, g1, g2], axis=0)

    # ==================================================================
    # Recurrent observation coefficients
    # Shape: (3, nb_tracks, nb_states, 1)
    # Only Gaussian 0 depends on the observation: [-1/LocErr, 0, 0]
    # ==================================================================
    obs_g0   = (-1.0 / LocErr_b)[..., None]                   # (nb_tracks, nb_states, 1)
    obs_zero = zeros[..., None]
    obs_vars = tf.stack([obs_g0, obs_zero, obs_zero], axis=0)
    
    # ==================================================================
    # Initial hidden-variable coefficients
    # Shape: (2, nb_tracks, nb_states, 2)
    # ==================================================================
    # Initial Gaussian 0 -- prior on initial position:  [1/spread, 0]
    init_g0 = tf.stack([1.0 / initial_position_spread, zeros], axis=-1)

    # Initial Gaussian 1 -- prior on initial anomalous variable
    #   confined :  [ 1/well_distance, -1/well_distance ]
    #   directed :  [            1e-15,             1/v ]
    ...
    initial_hidden_vars = tf.stack([init_g0, init_g1], axis=0)
    
    # ==================================================================
    # Transition hidden-variable coefficients
    # Shape: (1, nb_tracks, nb_states, 2)
    # Same form as the second initial Gaussian: re-initialises the
    # anomalous variable when a state transition occurs.
    # ==================================================================
    transition_hidden_vars = init_g1[None]
    
    # ==================================================================
    # Unit-std / zero-bias scaffolding tensors -- same semantics and
    # shape conventions as in the original function, but with the
    # ``nb_tracks`` axis pre-populated.
    # ==================================================================
    Gaussian_stds            = tf.ones((nb_obs_vars + nb_hidden_vars,
                                        nb_tracks, nb_states, 1), dtype=dtype)
    biases                   = tf.zeros((nb_obs_vars + nb_hidden_vars,
    ...
    initial_biases           = tf.zeros((nb_transition_gaussians,
                                         nb_tracks, nb_states, nb_dims), dtype=dtype)
    transition_Gaussian_stds = tf.ones((nb_transition_gaussians,
                                        nb_tracks, nb_states, 1), dtype=dtype)
    transition_biases        = tf.zeros((nb_transition_gaussians,
                                         nb_tracks, nb_states, nb_dims), dtype=dtype)
...
def RNN_cell(input_i, Prev_coefs, Prev_biases, LP, segment_len, reshaped_Log_factors, reshaped_transition_Log_factors, reccurent_obs_var_coefs, reccurent_hidden_var_coefs, reccurent_next_hidden_var_coefs, reccurent_biases, transition_hidden_var_coefs, transition_biases, sequence_phase_1, sequence_phase_2, transition_mask, transition_sequence, transition_mean, transition_var, gamma_dist_mean, gamma_dist_var, states, dt_ratios):
    ...
    current_segment_len = tf.concat([tf.ones((nb_tracks, nb_states), dtype = dtype), stable_segment_len + additional_stable_segment_len], axis = 1)
...
class Custom_RNN_layer(tf.keras.layers.Layer):
    ...
    @tf.function(jit_compile=False)
    def call(self, inputs, input_dts, reference_dt, mask, Prev_coefs, Prev_biases, LP, Log_factors, transition_Log_factors,
        ...
        transition_shapes, transition_rates = self.transition_param_function(
            transition_shapes, transition_rates, density, Fs, effective_ds, input_dts, reference_dt, dtype)
        '''
        ...
        reshaped_transition_Log_factors = (oh_src[None, None] @ transition_Log_factors[:,:, None])[:, 0][None]
        reshaped_Log_factors            = (oh_row @ Log_factors[:, None])[:, 0][None]
        # new versions:
        reshaped_transition_Log_factors = tf.math.reduce_sum(oh_src[None, None] * transition_Log_factors[:,:, None], -1)
        reshaped_Log_factors            = tf.math.reduce_sum(oh_row[None, None] * Log_factors[:,:, None], -1)
...
class Custom_RNN_layer(tf.keras.layers.Layer):
    ...
    def call(self, inputs, input_dts, reference_dt, mask,
        ...
        # ------------------------------------------------------------------
        segment_len     = tf.ones((nb_tracks, sequence_length * nb_states),
                                  dtype=dtype)
        gamma_dist_mean = transition_mean_full[0]                   # (N, P)
        gamma_dist_var  = transition_var_full[0]                    # (N, P)
        
        if self.carryover:
            segment_len     = (isfirst[:, None] * segment_len
                               + (1 - isfirst[:, None])
                                  * self.carryout_segment_len)
            gamma_dist_mean = (isfirst[:, None] * gamma_dist_mean
                               + (1 - isfirst[:, None])
                                  * self.carryout_gamma_dist_mean)
            gamma_dist_var  = (isfirst[:, None] * gamma_dist_var
                               + (1 - isfirst[:, None])
                                  * self.carryout_gamma_dist_var)
...
def build_abrupt_directed_motion_changes_model(track_len, # maximum number of time points in the input tracks 
    ...
    # inputs = sliced_inputs
    # mask = sliced_mask
    states = layer(sliced_inputs, inupt_dts, reference_dt, sliced_mask, Prev_coefs, Prev_biases, LP, Log_factors, transition_Log_factors, reccurent_obs_var_coefs, reccurent_hidden_var_coefs, reccurent_next_hidden_var_coefs, reccurent_biases, transition_hidden_var_coefs, transition_biases, log_ds, softmax_inv_Fractions, anomalous_factors, isdir)
